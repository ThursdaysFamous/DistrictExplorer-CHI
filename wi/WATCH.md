...
| Unscheduled — the city republishes when the facts move | The **Milwaukee city pair** (`mpd-district`, `milwaukee-neighborhoods`) is PRE-BUILT by `wi/scripts/build_milwaukee_city_layers.py` from city surfaces that change without a calendar: MPD redraws districts when it reorganizes (not on the census cycle), and the neighborhood map moves rarely if ever. Nothing weekly to review — the monthly source report watches both ArcGIS doors and both CKAN dataset pages, and a count or reachability change there is the trigger to re-run the builder, read the diff, and bump the sw cache. The builder's witnesses (exact key sets 1-7 / 190 count, area shares within 0.5%, the space-folded neighborhood key) are what catch a republication that changed shape rather than content | 2026-08-26 (first build: 7 districts at 0.04% max share difference, 190 neighborhoods at 0.007% — one spelling divergence on the record) |
