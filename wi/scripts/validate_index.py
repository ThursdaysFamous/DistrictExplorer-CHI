...
ROSTER_FILES = {
    ...
    "county-board-members.json": 720,  # County board supervisors for the 34 counties whose district-keyed member list this can obtain (750 seats) — 31 scraped from board pages, Milwaukee and Racine read off their own GIS layers (Milwaukee Legistar-witnessed). Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry; refreshed weekly by update-wi-county-board-roster.yml, and Taylor carried from its own directory page as a dated document because its host answers a captcha to every automated client. Sauk is the 31st page county, read by its own field LABELS rather than by adjacency, and witnessed three ways: a first.last county mailbox on every row against that row's name, its ward composition against LTSB's ward-level SUPERID, and the Clerk's candidate filing list. Contact rides only where its county published it — 86 mailboxes, 46 phones, 18 profile pages. The other 38 counties name nobody — their cards link the county board instead.
