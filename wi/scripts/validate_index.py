...
ROSTER_FILES = {
    ...
    "county-board-members.json": 1180,  # County board supervisors for the 54 counties whose district-keyed member list this can obtain (1,220 seats, 1,210 named and 10 the counties themselves mark vacant) — 42 read from each county's own board page, Milwaukee and Racine off their own GIS layers (Milwaukee Legistar-witnessed on every run), Fond du Lac through the Internet Archive with the county asked first and the capture's age gated, Dodge from its paginated constituent directory, Kenosha's and Adams's from directory PDFs, Jackson's from the roster PDF its own listing page links (discovered each run, since the filename carries the board's term, and witnessed against LTSB's ward filing), Waupaca's from the Clerk's Directory of Public Officials on the county's own host (its board page names nobody), Columbia's from the table its listing page frames from a second county host, and Taylor, Lafayette and La Crosse as dated documents behind challenges this project does not defeat. Calumet, Buffalo, Jackson and Waupaca (2026-08-31) are the 51st to 54th, all four recovered from this project's own record rather than from anything a county changed. Roles pass a uniqueness gate, because Calumet's page labels two supervisors Vice-Chairperson where its own minutes name one. Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry and refused outright if a county that shipped last week resolves nothing this week. The other 18 counties name nobody — their cards link the county board instead.
