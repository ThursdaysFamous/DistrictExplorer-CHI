...
ROSTER_FILES = {
    ...
    "county-board-members.json": 715,  # County board supervisors for the 34 counties whose district-keyed member list this can obtain (744 seats) — 31 scraped from board pages, Milwaukee and Racine read off their own GIS layers (Milwaukee Legistar-witnessed). Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry; refreshed weekly by update-wi-county-board-roster.yml, and Taylor carried from its own directory page as a dated document because its host answers a captcha to every automated client. Manitowoc (2026-08-29) is the first whose list LINKS a page per supervisor: those pages state the district a second time (a disagreement fails the county), carry a county e-mail behind two layers of the page's own obfuscation, and print the home address and home telephone this fleet never publishes. The other 38 counties name nobody — their cards link the county board instead.
