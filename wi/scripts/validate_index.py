...
ROSTER_FILES = {
    ...
    "county-board-members.json": 1121,  # County board supervisors for the 51 counties whose district-keyed member list this can obtain (1,160 seats, 1,150 named and 10 the counties themselves mark vacant) — 41 read from each county's own board page, Milwaukee and Racine off their own GIS layers (Milwaukee Legistar-witnessed on every run), Fond du Lac through the Internet Archive with the county asked first and the capture's age gated, Dodge from its paginated constituent directory, Kenosha's and Adams's from directory PDFs, Columbia's from the table its listing page frames from a second county host, and Taylor, Lafayette and La Crosse as dated documents behind challenges this project does not defeat. Calumet (2026-08-31) is the 51st and reads a district per TABLE CELL across four client-side tabs, one fetch carrying all of them; its roles pass a uniqueness gate, because the county's own page labels two supervisors Vice-Chairperson and its organisational minutes name only one. Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry and refused outright if a county that shipped last week resolves nothing this week. The other 21 counties name nobody — their cards link the county board instead.
