...
ROSTER_FILES = {
    ...
    "county-board-members.json": 1232,  # County board supervisors for the 56 counties whose district-keyed member list this can obtain (1,272 seats, 1,262 named and 10 the counties themselves mark vacant) — 42 read from each county's own board page, Milwaukee and Racine off their own GIS layers (Milwaukee Legistar-witnessed on every run), Fond du Lac through the Internet Archive with the county asked first and the capture's age gated, Dodge from its paginated constituent directory, Kenosha's and Adams's from directory PDFs, Jackson's from the roster PDF its own listing page links (discovered each run, witnessed against LTSB's ward filing), Waupaca's from the Clerk's Directory of Public Officials on the county's own host, Door's and Oconto's from CivicPlus staff-directory cards (Door gated on the widget header, the job title and a districtN@ mailbox all naming one district; Oconto's phones fetched from each member's own page and its courthouse switchboard dropped from all 31), Columbia's from the table its listing page frames from a second county host, and Taylor, Lafayette and La Crosse as dated documents behind challenges this project does not defeat. Calumet, Buffalo, Jackson, Waupaca, Door and Oconto (2026-08-31) are the 51st to 56th, all six recovered from this project's own record rather than from anything a county changed. Roles pass a uniqueness gate, because Calumet's page labels two supervisors Vice-Chairperson where its own minutes name one. Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry and refused outright if a county that shipped last week resolves nothing this week. The other 16 counties name nobody — their cards link the county board instead.
