...
GEOMETRY_FILES = {
    ...
    "aldermanic-districts.json": (853, 853),  # Every aldermanic (and village trustee) district the ward fabric composes completely — coded city/village wards dissolved on the statewide municipality key by wi/scripts/build_wi_aldermanic_districts.py under its completeness, BAS-witness and point-agreement gates (853 districts, 156 municipalities; nine incomplete submissions still excluded on the record). Appleton's fifteen are LOCALLY COMPOSED: its county files every ward uncoded, so the city clerk's own polling-locations page supplies the ward-to-district assignment, gated against a second edition of that same assignment in the state's polling file, against the ten wards the state does code, and against Census 2020 population balance. Rebuild after each Jan/Jul filing window; a count change is expected news.
    ...
    "law-service-areas.json": (639, 639),  # 639 law-enforcement dispatch areas — the OEC's 3,083 effective NG911 LawEnforcementBoundary polygons dissolved per agency; plain -dissolve, never -dissolve2, so the concurrent-jurisdiction overlaps the counties filed survive (measured). Same builder and gates as the fire file, staleness included.
...
ROSTER_FILES = {
    ...
    "wi-alderpersons.json": 18,  # The alderperson roster for the 18 municipalities with a verified, witnessed route — the six big cities of 2026-08-26 (Milwaukee, Madison, Green Bay, Kenosha, Racine, Waukesha) plus the twelve of 2026-09-05 (Stevens Point, Menomonie, Manitowoc, Sheboygan, Superior, Portage, Viroqua, Menasha, Howard, Tomah, Eau Claire, Appleton), 208 seats — keyed by the municipality's statewide COUSUBFP and zero-padded district id, the geometry file's exact key pair. Built by wi/scripts/build_wi_alderperson_roster.py with per-municipality, per-field floors and a geometry cross-gate; a municipality whose page could not be read keeps its last shipped rows and carries a carriedFrom date the card renders, so nothing rides stale silently. Refreshed weekly by update-wi-alderperson-roster.yml.
