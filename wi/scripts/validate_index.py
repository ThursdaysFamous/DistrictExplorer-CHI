...
ROSTER_FILES = {
    ...
    "county-board-members.json": 1135,  # County board supervisors for the 52 counties whose district-keyed member list this can obtain (1,174 seats, 1,164 named and 10 the counties themselves mark vacant) — 42 read from each county's own board page, Milwaukee and Racine off their own GIS layers (Milwaukee Legistar-witnessed on every run), Fond du Lac through the Internet Archive with the county asked first and the capture's age gated, Dodge from its paginated constituent directory, Kenosha's and Adams's from directory PDFs, Columbia's from the table its listing page frames from a second county host, and Taylor, Lafayette and La Crosse as dated documents behind challenges this project does not defeat. Calumet and Buffalo (2026-08-31) are the 51st and 52nd, both recovered from this project's own record rather than from anything a county changed: Calumet reads a district per table cell across four client-side tabs, Buffalo an Elementor heading pair per supervisor with Cloudflare-obfuscated addresses decoded and floored. Both pass a role uniqueness gate, because Calumet's page labels two supervisors Vice-Chairperson where its own minutes name one. Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry and refused outright if a county that shipped last week resolves nothing this week. The other 20 counties name nobody — their cards link the county board instead.
