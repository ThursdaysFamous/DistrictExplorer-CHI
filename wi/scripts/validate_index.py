...
ROSTER_FILES = {
    ...
    "county-board-members.json": 709,  # County board supervisors for the 34 counties whose district-keyed member list this can obtain (738 seats) — 31 scraped from board pages, Milwaukee and Racine read off their own GIS layers (Milwaukee Legistar-witnessed). Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry; refreshed weekly by update-wi-county-board-roster.yml, and Taylor carried from its own directory page as a dated document because its host answers a captcha to every automated client. Green Lake is read as structured BLOCKS rather than by proximity, which is what lets its supervisors ship with the role, e-mail and phone the county prints inside each block. The other 38 counties name nobody — their cards link the county board instead.
