...
ROSTER_FILES = {
    ...
    "county-board-members.json": 645,  # County board supervisors for the 31 counties that publish a district-keyed member list a client can read (672 seats) — 29 scraped from board pages, Milwaukee and Racine read off their own GIS layers (Milwaukee Legistar-witnessed). Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry; refreshed weekly by update-wi-county-board-roster.yml. The other 41 counties name nobody — their cards link the county board instead.
