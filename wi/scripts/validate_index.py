...
ROSTER_FILES = {
    ...
    "county-board-members.json": 719,  # County board supervisors for the 34 counties whose district-keyed member list this can obtain (748 seats) — 31 scraped from board pages (Rock's from the Internet Archive, its own host refusing this client), Milwaukee and Racine read off their own GIS layers (Milwaukee Legistar-witnessed). Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry and refusing to write when a county that shipped before resolves nothing; refreshed weekly by update-wi-county-board-roster.yml, and Taylor carried from its own directory page as a dated document because its host answers a captcha to every automated client. The other 38 counties name nobody — their cards link the county board instead.
