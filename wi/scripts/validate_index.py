...
ROSTER_FILES = {
    ...
    "county-board-members.json": 1244,  # County board supervisors for the 57 counties whose district-keyed member list this can obtain (1,284 seats, 1,274 named and 10 the counties themselves mark vacant) — 41 read weekly from each county's own host, in six page shapes with each county's reading direction pinned (Waupaca's is the Clerk's Directory of Public Officials, Door's and Oconto's are CivicPlus staff-directory cards); Milwaukee and Racine off their own GIS layers (Milwaukee Legistar-witnessed on every run); Fond du Lac through the Internet Archive with the county asked first and the capture's age gated; Dodge from its paginated constituent directory; Kenosha's and Adams's from directory PDFs; Columbia's from the table its listing page frames from a second county host; and NINE carried as dated documents that are never re-fetched: Taylor, Lafayette and La Crosse behind a captcha or a Cloudflare challenge, and Jackson, Richland, Rusk, Polk, Dunn and Pepin because each of those sites publishes a robots.txt disallowing the whole site to every agent it does not name. Five of the six were scraped weekly until 2026-08-31, when wi/scripts/validate_robots.py swept every host this instance fetches; the crawl stopped and the dated names stayed, and their cards say the county ASKS rather than refuses. Calumet, Buffalo, Jackson, Waupaca, Door, Oconto and Pepin (2026-08-31) are the 51st to 57th. Roles pass a uniqueness gate, because Calumet's page labels two supervisors Vice-Chairperson where its own minutes name one. Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry and refused outright if a county that shipped last week resolves nothing this week. The other 15 counties name nobody — their cards link the county board instead.
