...
ROSTER_FILES = {
    ...
    "county-board-members.json": 680,  # County board supervisors for the 33 counties whose district-keyed member list this can obtain (709 seats) — 29 scraped from board pages, Milwaukee and Racine read off their own GIS layers (Milwaukee Legistar-witnessed), Taylor carried from its own directory page as a dated document because its host answers a captcha to every automated client, and Adams re-read weekly from the county clerk's own published directory PDF (the link is resolved each run rather than the file id pinned, because the clerk republishes each edition under a new one). Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry; refreshed weekly by update-wi-county-board-roster.yml. The other 39 counties name nobody — their cards link the county board instead.
