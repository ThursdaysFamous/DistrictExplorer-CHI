...
ROSTER_FILES = {
    ...
    "county-board-members.json": 674,  # County board supervisors for the 32 counties that publish a district-keyed member list a client can read (701 seats) — 30 scraped from board pages (Rock's from the Internet Archive, its own host refusing this client), Milwaukee and Racine read off their own GIS layers (Milwaukee Legistar-witnessed). Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry and refusing to write when a county that shipped before resolves nothing; refreshed weekly by update-wi-county-board-roster.yml. The other 40 counties name nobody — their cards link the county board instead.
