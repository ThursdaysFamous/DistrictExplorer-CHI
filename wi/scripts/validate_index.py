...
ROSTER_FILES = {
    ...
    "county-board-members.json": 713,  # County board supervisors for the 34 counties whose district-keyed member list this can obtain (742 seats) — 30 scraped from board pages, Milwaukee and Racine read off their own GIS layers (Milwaukee Legistar-witnessed). Built by wi/scripts/build_wi_county_board_roster.py, checked seat-for-seat against the shipped district geometry; refreshed weekly by update-wi-county-board-roster.yml. TWO counties come from documents and they are opposite arrangements: Kenosha's Directory of Public Officials is FETCHED and witnessed against the county board page on every run, while Taylor is CARRIED from its own directory page as a dated, not-re-read document because its host answers a captcha to every automated client. The other 38 counties name nobody — their cards link the county board instead.
