    ...
    for fips, entry in sorted(counties.items()):
        ...
        for d, member in entry["districts"].items():
            ...
            if member["vacant"]:
                ...
                # contact rides only where its county published it beside the
                # seat: Milwaukee's and Racine's GIS features carry it as
                # attributes, Taylor's directory and Monroe's table print a
                # county address per district. A PHONE is deliberately not
                # carried anywhere — Taylor and Monroe both publish personal
                # numbers next to personal addresses, and no card in this
                # instance has a phone row to put one in.
                if member.get("email"):
