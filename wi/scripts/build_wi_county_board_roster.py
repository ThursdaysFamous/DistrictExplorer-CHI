...
this can obtain; the other 38 are recorded in the Data gaps panel and their
cards keep linking the county board rather than naming anybody. See the
scraper's docstring for what each of the other 38 actually publishes, and for
the four routes beyond a plain page read (two county GIS layers, Taylor's
dated document, and the table Columbia's listing page frames from a second
host).
...
MIN_COUNTIES = 32      # 34 ship one (30 pages + 2 county GIS layers + Taylor by document + Columbia's framed table); tolerates two dark
MIN_SEATS = 718        # 747 today (663 page-scraped + Milwaukee 18 + Racine 21 + Taylor 17 + Columbia 28)
