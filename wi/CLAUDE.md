...
districtry Wisconsin: a single-file, dependency-light web app. Click a point in Wisconsin (or search an address) and it reports every civic district containing that point and who represents you there. It serves at **districtry.com/wi/** as a folder of the consolidated districtry repo — the first state to expand **in place** rather than as a fork (the retired template route proved this tree end-to-end on the archived `DistrictExplorer-WI` repo before the fleet consolidated; `docs/EXPANSION_GUIDE.md` §4.10 is the decision record). It ships twenty-three layers — phase 1's twelve, phase 2's seven (`docs/WI_PHASE2_PLAN.md`, delivered 2026-08-26), and phase 3's first four. **MPS School Board District** is the fleet's second elected school board and this instance's first city-scoped, coverage-gated layer: statute names Wisconsin's districted school boards exactly (MPS and Racine Unified — RUSD's unpublished geometry is gap `rusd-school-board`), the eight districts come from Milwaukee's own layer witnessed against the city's open-data shapefile, and all nine directors ship from the district's own page under a committee-list witness (`wi/scripts/build_mps_school_board_districts.py`, `mps_school_board_scraper.py`). And **Aldermanic District**, 838 city and village council districts dissolved from LTSB's coded wards on the statewide municipality key (never the county-qualified `ALDER_FIPS`, which splits cross-county cities), witnessed against the state's own BAS pre-dissolve edition key for key, with ten measurably incomplete county filings excluded on the record and alderpersons named in six big cities from their own witnessed surfaces (`wi/scripts/build_wi_aldermanic_districts.py`, `wi_alderperson_scraper.py` — whose Kenosha canvass witness caught the city GIS stale on one seat at first build, and whose Madison read survives the index page's off-by-one flat-pairing trap around the vacant seat). Thirteen ride national publishers: U.S. House, WI Senate, WI Assembly, County, the three TIGER school-district tilings (Unified / Union High / Elementary), County Subdivision, Municipality, ZIP Code (ZCTA), Post Office, and the USGS-structures Police and Fire Station nearest-3 pair. Six come from state sources, and each carries a lesson: **County Board District** — all 1,590 supervisory districts in one fetch, because Wis. Stat. 5.15(4)(br)1 makes every county file its boundaries with LTSB each 15 January and 15 July and LTSB publishes the aggregate (Trempealeau's 17 come from the county's own service, because LTSB's file merges two of them; `wi/scripts/build_wi_supervisory_districts.py` carries the measurement) — supervisors are NAMED in the 22 counties that publish a district-keyed roster (20 board pages + the Milwaukee and Racine county GIS layers, 476 seats, weekly CI), and the 50 that publish none are the standing Data-gaps entry; **Municipal Ward** — LTSB's live 7,161-ward layer, the ballot sub-unit everything else is composed from; **Circuit Court** and **Court of Appeals District** — county unions under a double witness (Wis. Stat. 753.06 / 752.11 AND wicourts.gov agreeing), both benches scraped weekly from the court system itself; and the DPI-directory **School Location** and **Library** nearest-3 points, statewide where the reference instance's library card is city-scoped. The county card names each county's **clerk** from the Blue Book cross-gated against the clerks' own association (WEC itself sits behind a Cloudflare challenge — recorded, never defeated), and both chambers carry the legislature's own office data. Phase 3's other two are Milwaukee's own city tier, both on the same `milwaukeeCoverage` gate as the MPS board and both built by one script with the MPS build's two-surface machinery (`wi/scripts/build_milwaukee_city_layers.py` — the city's ArcGIS layer server-reprojected, the CKAN shapefile as the build-time area-share witness): **Police District (Milwaukee)**, seven MPD districts that revisit the safety matrix's statewide drop with the city-scoped frame Chicago's own layer uses — the captains are gap `mpd-district-leadership`, because city.milwaukee.gov's Cloudflare challenge is an access control and the card therefore links MPD's district pages and names no one — and **Neighborhood (Milwaukee)**, 190 identity-only polygons whose all-caps names ship title-cased with the raw value kept, and whose witness caught the city's two copies spelling one neighborhood apart (service MCGOVERN PARK, shapefile MC GOVERN PARK — the service spelling ships). Everything beyond these is this instance's next growth per `docs/EXPANSION_GUIDE.md` (Part 2 for county-by-county builds, §4.3 for new layer modules); the remaining phase-3 candidates (MPD squad areas, the city TID dataset, the NG911 service boundaries) sit researched in `docs/DATA_LAYER_GUIDEBOOK.md`'s backlog.
