...
```json
{
  "chicago": [
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "county-board",
      "summary": "Hardin County's commissioners aren't named. They're elected county-wide, so there are no districts to draw — only the list of who holds the seats is missing.",
...
**HARDIN gets a second witness and still does not ship**, and the distinction is
worth stating because both counties rest on the same evidence class. Its platinum
2016 canvass reports `For County Commissioner` over 6 of 6 precincts with
DAVID W ROBINSON beating ARCHIE "RUSTY" CORNELL — two independent vendors, two
elections a decade apart, both countywide. Its census fabric passes too. What it
lacks is Pulaski's second naming source: the platinum canvass carries no
committeeperson contests, so Hardin's precinct *names* rest on the pollresults
feed alone. A county joining on precincts alone should have the footing Pulaski
has, so this is recorded and not built.
