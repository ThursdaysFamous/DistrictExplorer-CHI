...
```json
{
  "chicago": [
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "municipality",
      "summary": "Boone's five municipalities show a name and a link only \u2014 but the clerk's yearbook that covers all five was finally refreshed on 4 Aug 2026 and now post-dates the April 2025 election, so this is buildable work rather than a missing source.",
      "blocker": "The clerk's \"Boone County, Illinois Year Book\" was always the right kind of source \u2014 all five municipalities, and Belvidere's alderpersons by ward with contact details \u2014 but the newest edition posted was 2024, which named three Belvidere aldermen the April 2025 election replaced. RE-MEASURED 2026-08-20 AND THE SOURCE HAS ARRIVED: the Clerk posted BOTH a 2025 and a 2026 edition on 2026-08-04, one day after this project settled the fire-district record and four days after the park/library re-check, so the gap missed them by days rather than by months. The 2026 edition is a 70-page PDF with a REAL TEXT LAYER (about 114,000 characters, no scanned pages in the municipal section), and the three superseded aldermen are gone: the 2025 and 2026 editions carry the April-2025 winners with 2029 terms. Coverage measured section by section: Belvidere (mayor, clerk/collector, treasurer, ten alderpersons BY WARD, plus appointed department heads), Caledonia, Capron, Poplar Grove and Timberlane, each with president, clerk, treasurer and trustees, vacancies printed as vacancies. TWO THINGS A BUILDER MUST KNOW. THE YEARBOOK PRINTS HOME ADDRESSES \u2014 54 street-address lines in the municipal section, of which only the section headers are municipal offices and every per-person address is a residence, so the Madison/Peoria rule applies and the parser must take name, office, ward, term, phone and e-mail while never touching the address column. And the ward labels sit in a separate text column from the member rows, so a flat text extraction mis-orders them (it emits \"Fifth:\" after that ward's members); a layout-aware read keyed on the x-positions recovers the assignment cleanly. WHAT IT ACTUALLY ADDS, since Belvidere's aldermen already ship from the county's ward service: the yearbook corroborates all ten names and wards exactly, ADDS an e-mail address for each (which the ward service does not carry), and closes the real remaining hole, which is Caledonia, Capron, Poplar Grove and Timberlane plus Belvidere's citywide officers. One disagreement to resolve rather than average: Sandra Gramkowski's phone is 815-289-0493 in the yearbook and 815-298-0493 in the ward service, a transposition in one of the two, and neither source is obviously right.",
      "wanted": "Nothing from a publisher \u2014 the source exists and is current. What is left is the build: a layout-aware yearbook parser that keeps home addresses out by construction, and a decision on the one transposed phone number."
    },
    {
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "county-precinct",
      "summary": "Calhoun County's five voting precincts are BUILDABLE from the census fabric \u2014 each is a union of whole townships named in its own title, and the county's returns prove the composition. Only a precinct-level variant of the fabric check stands in the way.",
      "blocker": "Checked 2 Aug 2026 when the county was added: Calhoun runs no mapping system, nothing for the county appears in any public map catalogue, and the clerk's election files are in an unusual form \u2014 the 2026 primary summary being a raw printer file rather than a PDF. RE-MEASURED 2026-08-20, AND BOTH HALVES OF THAT READ THE WRONG WAY ROUND. The \"raw printer file\" is HP PCL, which is ASCII with escape sequences \u2014 strip the escapes and the precinct report is cleaner text than the PDFs this fleet parses routinely, and three consecutive elections' reports agree precinct for precinct. The county's site (calhouncountyil.gov, confirmed three ways including the clerk address this repo already ships) serves its Elections page as text through the WordPress REST API, and ISBE's statewide precinct-level results CSV carries the same five names. So the county publishes no GEOMETRY, which was true, but it publishes its precinct list plainly, which the record implied it did not. THE CENSUS FABRIC ANSWERS THE GEOMETRY: 7 Census 2020 voting districts whose populations sum to 4,437, the county's exact Census 2020 count, and every one of the seven is a whole township at an intersection-over-union of 1.000000 (CRATER-CARLIN being the union of Crater and Carlin). Between November 2022 and November 2024 the county merged 7 precincts into 5 BY UNITING WHOLE ONES, and it wrote the composition into the names: Belleview-Hamburg is BELLEVIEW plus HAMBURG, Hardin-Gilead is HARDIN plus GILEAD, and the county's own certified registration figures confirm each merged precinct equals the sum of its two named parts within ordinary two-year drift. Nothing is guessed. WHAT ACTUALLY BLOCKS IT IS OUR OWN GATE, not the county: check_fabric tests the precinct NAME SET against the VTD name set, which is the right test for the Clark shape (one precinct per VTD) and the wrong one here (one precinct per SEVERAL VTDs). Run with the composition supplied, every other gate already passes \u2014 the partition claims each of the 7 VTDs exactly once, the dissolve tiles the county with 0.000 square metres of overlap and 100.000000% coverage, and every VTD lands inside its own precinct.",
      "wanted": "Nothing from a publisher. This one is ours to build: a precinct-level sibling of check_fabric that tests the CONSUMED VTD set against the census set rather than matching names one-for-one, keeping the population-identity check as it is. The 7-into-5 whole-precinct merge shape will recur, so it belongs in the shared module rather than in one county's builder."
    },
    {
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "county-board",
      "summary": "Five county boards name an address on their card and 47 name none \u2014 Cook, Lake and Coles give an office, Clark and White a mailing line.",
    ...
    },
    {
      ...
      "kind": "data-quality",
      "layer": "county-precinct",
      "summary": "Tazewell's six unanswered precincts now show a polling place from the Clerk's own listing; the county fixed two of the five disagreements the same week, and THREE remain.",
      "blocker": "Checked 2 Aug 2026, when the county was added: 76 of the 82 precincts match a published polling place. The six that do not — Elm Grove 02, Cincinnati 02 and 05, Pekin 01 and 06, and Morton 01 — point at three building IDs that appear on no published county dataset. That is a gap in the county's own records, not in how we match them. Everything else on those cards still shows. ANSWERED 17 Aug 2026: Deputy Clerk Reynolds sent the Clerk's own polling-place listing for the 2026 General Primary — 19 townships, 82 precincts, 49 polling places, every precinct paired with a named building and address, archived from her e-mail. All six orphans are in it: Elm Grove 02 → Grace Baptist Church; Cincinnati 02 and 05 → Pekin Bible Church; Pekin 01 → First Christian Church; Pekin 06 → Miller Center; Morton 01 → Eastside Bible Church. BUILT AND SHIPPED THE SAME DAY as tazewell-precinct-polling.json, so all 82 precincts now name a polling place. TWO THINGS THE BUILD MEASURED THAT NOBODY HAD ASKED, and they are why this record NARROWS rather than closes. (1) THE FACILITY IDS ARE AMBIGUOUS, not merely absent: id 50 is referenced by Cincinnati 02 and 05, which vote at Pekin Bible Church, AND by Elm Grove 02, which votes at Grace Baptist Church; id 54 by Pekin 01 (First Christian Church) AND Pekin 06 (Miller Center). Precincts voting at different buildings share one id, so the Whiteside facility-keyed shape could not express the answer and this file is keyed by PRECINCT NAME instead — and publishing three building records upstream would NOT fix the county's layer, which is what the operator's 17 Aug note to the Clerk assumed. (2) FIVE PRECINCTS WHERE THE COUNTY'S TWO SURFACES DISAGREE outright, found by comparing all 82 against the Clerk's listing: Pekin 05 (county layer says Miller Center, the Clerk's list says Pekin Bible Church); Groveland 02 and 04 (Creve Coeur Village Hall and Community Center, SWAPPED between the two sources); and Washington 05 and 12 (the county layer says Spring Lake Town Hall — a building in Manito, in a different township — where the Clerk's list says John L Hensey Elementary School). Nine further name differences were checked and are the same building worded differently, not disagreements. NOTHING WAS OVERRIDDEN: the county's layer was itself edited 2026-08-14, three days BEFORE the Clerk's e-mail, so neither source is stale and picking one would be guessing which county surface is right. Those five keep showing the county's published answer until the Clerk says which is current.  PARTLY FIXED 2026-08-18, and re-measured rather than taken on trust. Deputy Clerk Reynolds routed the five disagreements to the county's GIS at 13:06Z and wrote back at 15:19Z that he “believes he has it taken care of”; the feature service's own lastEditDate confirms an edit at 15:12Z, seven minutes earlier. Re-querying both layers with cache-busting headers shows what actually changed: GROVELAND 02 AND 04 ARE FIXED — the swap is gone and both now agree with the Clerk's list. THREE DISAGREEMENTS SURVIVE, and they are the consequential ones: Pekin 05 still points at the Miller Center where the Clerk's list says Pekin Bible Church, and Washington 05 and 12 still point at SPRING LAKE TOWN HALL — a building in Manito, roughly twenty miles away in another township — where the Clerk's list says John L Hensey Elementary School in Washington. THE THREE MISSING FACILITY IDS (43, 50, 54) ARE ALSO UNCHANGED: six precincts still match no published location, so data/app/tazewell-precinct-polling.json is still doing its job and is not retired. Six further name differences were re-checked and remain the same building worded differently (“Pekin City Bus Department” vs “City of Pekin Bus Dept.” and the like), plus Tremont 02, where the layer says Allentown Union Hall and the Clerk says Allentown Town Hall — which may be one building under two names and is NOT counted as a disagreement without knowing. Nothing was overridden, on the same reasoning as before.",
      "wanted": "Confirmation from the Clerk's office of which source is current for the three precincts that still disagree — Pekin 05, and Washington 05 and 12 — with Washington the urgent one, since the county's layer would send a Washington voter to another township. And, upstream, facility ids that identify one building each (43, 50 and 54 still resolve to nothing), so the county's own precinct and voting-location layers can agree without a supplement. Groveland 02 and 04 were fixed by the county on 2026-08-18 and are no longer part of this gap; the six unanswered precincts remain shipped from the Clerk's list."
    },
    {
...
**What it can do, measured:** 52 served counties have a districted board; 25 already badge
a chair from their own county's source and **27 do not — and ISBE names a chair for 24 of
those 27** (Adams, Carroll, Clark, Coles, DeKalb, Edgar, Effingham, Henry, Jefferson, Jo
Daviess, Kankakee, Lee, Livingston, Madison, McLean, Montgomery, Ogle, Sangamon,
Stephenson, Washington, White, Whiteside, Winnebago, Woodford). Spot-checked against what
this app already ships: Clark's chair, Rex Goble, is one of the seven members the card
names from certified canvasses — the state source and the county's returns agree.

**What it can never do:** name a board MEMBER. `"County Board Member"` occurs zero times in
the file and `"District"` zero times — it is an officers directory, not a roster, so it
closes no county-board roster gap on its own.

**Why it is not built yet, rather than an oversight.** A chair is an office that turns over
between a document's date and today, so this is the document-carried posture (Edwards /
Wabash `DOCUMENT_ROSTERS`): carry the source and its date, print a NOT RE-READ line, and
decide per card whether a dated state badge beats no badge. That is a deliberate build with
a fleet-wide surface, not a correction — and a second ISBE surface found in the same pass,
`ComplianceRecord.htm` (each county's board size, district structure and members-per-district;
Adams 21 seats / 7 multi-member districts / 3 each), is useful for the same work but carries
2007 metadata and no revision date, so it is citable for STRUCTURE and never as currency.
