...
```json
{
  "chicago": [
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "county-board",
      "summary": "Richland County's seven board districts aren't drawn, so the card can't say which one you're in. The county does name a member for every district.",
      "why": "The county's district map is a picture whose labels blur exactly where the lines cut through Olney, and its election results count each district's precincts without naming them.",
      ...
      "wanted": "Which precincts make up each of the seven districts — a table, the redistricting ordinance's description, or the map as data. Everything else is in hand."
    },
    {
...
| `platinumelectionresults.com` | 8 | per-precinct race pages — composes districts outright |
| `electionstats.hancockcounty-il.gov` | 1 (county-run, not a vendor) | per-contest PER-PRECINCT tables |
| `results.gbsvote.com` — *recorded in this guidebook's backlog 2026-08-20, first BUILT from 2026-08-21* | **13** — Cass, Cumberland, Fulton, Greene, Grundy, Jasper, Johnson, Knox, Morgan, Perry, Scott, Warren, Washington (+ Parke, Indiana) | **SUMMARY canvasses only**: a per-contest precinct COUNT, plus one committeeperson contest per precinct |
