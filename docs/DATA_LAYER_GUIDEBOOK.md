  ...
  "wisconsin": [
    {
      ...
      "kind": "no-source",
      "layer": "county-board",
      "summary": "Your county board district is named, but not the supervisor who holds it — the card links your county's board instead.",
      "why": "Wisconsin publishes every county's board district lines in one statewide file, but no statewide roster of the supervisors in them. Only each county publishes its own members, 72 different ways, so no name is shown rather than a guessed one.",
      "blocker": "Measured 2026-08-25, and narrowed from the import-time entry, which was about the DISTRICTS and is closed: LTSB's WI_County_Supervisory_Districts_Current carries all 1,589 districts across all 72 counties under Wis. Stat. 5.15(4)(br)1, and ships. What is genuinely absent is the ROSTER. Three statewide routes were checked and none answers: LTSB's own layers carry no member columns (only a CONTACT e-mail per county, which is the clerk or, for five counties, an outside engineering firm); the Wisconsin Counties Association publishes no machine-readable member list; and the ward layer carries district ids but no people. A 72-county sweep of the counties' own sites did establish the shape of the work — 23 counties publish a district-by-district member page a parser can read (Trempealeau's is the reference: Members / Phone / District / Term Ends), 3 more publish partial lists, and 44 could not be read by this client, of which 11 answer 403 to a datacenter client while serving browsers, 2 answer 503, one (Taylor) sits behind an sgcaptcha challenge that is an access control and is not defeated here, and the rest publish members as PDFs, images, or prose with no district column. So this is 72 separate builds, not one, and the honest interim is the county-board-directory link the card already carries.",
      "wanted": "A county's own board page that lists each supervisory district beside the supervisor holding it — one county is enough to start, and 23 counties already publish exactly that."
    }
  ]
}
```
...
| Elected school board (districted) | SHIPPED `school-board` (ERSB) | NO HONEST ANALOG³ | NO HONEST ANALOG (at-large board)⁴ | NOT SHIPPED — Wisconsin school boards are mostly elected AT LARGE; a districted minority would be a per-district build |
