...
```json
{
  ...
  "wisconsin": [
    ...
    },
    {
      ...
      "kind": "blocked",
      "layer": "ward",
      "summary": "Outside Milwaukee the ward card names your ward but not your polling place — the state publishes the pairing only behind a lookup that refuses automated clients, so the card links that lookup instead.",
