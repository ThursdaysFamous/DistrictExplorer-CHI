...
Fleet totals: **Chicago 39 · NYC 27 · SF 16 · Wisconsin 29** layers.
...
| Tax increment financing (TIF) district | SHIPPED `tif-district` (Cook, 418 — the Clerk's un-yeared current agency tiling, clerkTaxDistricts L18; dedicated Cook layer per the single-county rule until a second county ships — Kendall's `TIF_Districts` service is the recorded next entry) | n/a — New York State discontinued NYC-style TIF; no city program | n/a — SF uses IFDs/CFDs, no published district tiling evaluated | SHIPPED `tid-district` (79, 2026-08-26) — the recorded city-scoped candidate delivered under Wisconsin's own name (Wis. Stat. 66.1105 TIDs), on `milwaukeeCoverage` with IL tif-district parity throughout (geography group, the purple pair, name-primary card + district number + creation year, no elected body). The city's live layer serves only ACTIVE districts while its CKAN shapefile keeps all 56 retired ones — so the two-surface witness is SCOPED to the shapefile's own STATUS flag (79 at STATUS 1 match the live set exactly; the dissolve DATE is the wrong filter, since 15 retired rows carry none), and the builder drops any TID that gains a past dissolve date. The statewide reading stands: DOR publishes tabular TID data only, so this stays per-municipality |
...
| `tid-district` | Tax Incremental District (TID) | geography | Polygon | pre-built `tid-districts.json` — the city's 79 ACTIVE TIDs (dissolved dropped by date), witnessed against the CKAN shapefile scoped to the city's own STATUS flag — the shapefile keeps all 56 retired districts (build_milwaukee_city_layers.py; operator rebuild) | — (a finance overlay elects no one; card links the city's open-data TID page) | `milwaukeeCoverage`; inside the city most points are honestly in no TID |
