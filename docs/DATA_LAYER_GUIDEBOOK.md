...
- Collar-county boards — **complete (2026-07): all seven counties shipped.**
  ...
  **McHenry retro-debt: RESOLVED (2026-07)** — the board card now joins
  ...
  (per-member profile links the scraper already collected but the builder
  dropped; names re-verified via the directory's Archive snapshot — the
  county blocks live fetch). **Kendall carries the one open debt of that
  class**: its scraper, too, collects each member's profile URL and its
  builder drops it — carry `url` through `member_obj` as McHenry's does.
  Future counties join the consolidated `county-board` layer as dispatch
  entries, not new layers (`docs/COUNTY_LAYER_CONSOLIDATION.md`).
