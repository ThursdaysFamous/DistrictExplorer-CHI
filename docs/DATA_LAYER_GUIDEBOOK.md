...
| State upper chamber | SHIPPED `il-senate` | SHIPPED `state-senate` | SHIPPED `ca-senate` | SHIPPED `wi-senate` (33) — pre-built TIGERweb, Open States roster |
| State lower chamber | SHIPPED `il-house` | SHIPPED `state-assembly` | SHIPPED `ca-assembly` | SHIPPED `wi-assembly` (99) — pre-built TIGERweb, Open States roster |
...
| County clerk (surfaced on the county-identity card) | SHIPPED on `county` — all 101 clerk-authority counties via ISBE's election-authority directory (weekly CI; Peoria deliberately absent, its authority is an appointed election commission) | SHIPPED on `borough` — appointed (Appellate Division), labeled so; operator-verified `clerk` entries in `borough-officials.json` (nycourts.gov is Cloudflare-fronted, so no scraper; names only where the office's own page publishes one) | n/a⁹ | NOT SHIPPED — `county` is identity-only; the county's own site is linked from the `county-board` card instead. The route is verified (2026-08-25) and ships per `docs/WI_PHASE2_PLAN.md` PR 5: WEC itself is Cloudflare-challenged, so the sources are the Wisconsin Blue Book 2025-26 county-officers table (all 72 + party) cross-gated against wisconsincountyclerks.org's 72 per-county pages (name/address/phone/fax/email/website/hours; plain UA WAF, robots Crawl-delay 10 honored) — with the Milwaukee exception from Wis. Stat. 7.20(1): its county election authority is the APPOINTED county Election Commission, linked and labeled, never named |
...
| County | SHIPPED `county` (statewide IL) | SHIPPED `borough` (= county) | n/a — city and county are coterminous (recorded) | SHIPPED `county` (statewide WI, pre-built — the offline anchor; identity-only, see the county-clerk row) |
...
| `wi-senate` | Wisconsin State Senate District | political | Chamber | pre-built (TIGERweb L1 — the post-2024-remedial map, "Jan 1 2025 vintage", 10/10 LTSB agreement) | `wi-senate-members.json` (Open States, weekly CI) | — |
| `wi-assembly` | Wisconsin State Assembly District | political | Chamber | pre-built (TIGERweb L2, same vintage proof) | `wi-assembly-members.json` (Open States, weekly CI) | — |
...
| `county-board` | County Board District | political | Bespoke | pre-built `county-supervisory-districts.json` (LTSB statewide, 1,589 → 1,590 with the Trempealeau override; operator rebuild each Jan/Jul filing window) | `county-board-directory.json` (72/72, board size + link) + `county-board-members.json` (20 counties, 437 seats, weekly CI) | statewide; supervisor NAMED only inside the 20-county `metro-outline.json` ring |
...
| `county` | County | geography | Polygon | pre-built (TIGERweb State_County, 72 — offline anchor) | — (identity-only; clerk join ships per `docs/WI_PHASE2_PLAN.md` PR 5) | — |
