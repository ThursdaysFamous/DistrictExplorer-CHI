...
| Municipal governing body (surfaced on the municipality-identity card) | SHIPPED on `municipality` — suburban Cook's 128 municipalities (head of government + 785 board seats incl. 156 ward/district seats + clerks/treasurers + hall contact) joined by Census place GEOID from the County Clerk's Directory of Elected Officials API (weekly CI). Chicago is excluded by concept — its council is `ward`. The other six metro counties are sourced but unbuilt, and depth varies honestly by county (`docs/MUNICIPAL_COUNCILS_PLAYBOOK.md`); an unsourced municipality keeps the identity-only card | n/a (NYC's municipalities are the five boroughs — `borough-president`) | n/a (consolidated city-county) |
...
- Municipal governments (mayor / village president + city council / village board) —
  **Cook SHIPPED 2026-07; six counties sourced but unbuilt**
  ...
  twice: Will the Clerk's Directory flipbook (full governing body, 37 munis);
  ...
  honesty floor. Blocker for the six: six scrapers unbuilt, four of their sources are
  PDFs (introduces the repo's first PDF dependency, `pypdf`), and two counties
  (McHenry, Kendall) are Akamai-fronted and need the engine ladder. Statewide
...
| `municipality` | Municipality | geography | Bespoke | live TIGERweb Places | `municipal-officials.json` (weekly CI; suburban Cook only so far — head of government + board + other elected officers + hall contact, joined by place GEOID) | — |
