...
```json
{
  "chicago": [
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "fire-district",
      "summary": "Boone County's fire districts are not shown.",
      "blocker": "The county's fire district boundaries are numbered rather than named, and a card reading “Fire District 1” would tell a reader nothing they could act on.",
      "wanted": "A list matching Boone's fire district numbers to their names, or boundaries that carry the names."
    },
    {
    ...
    },
    {
      ...
      "blocker": "The county's mapping system publishes an officeholder dataset for each municipality, but for these two it carries the council seats only — there is no mayor or president to read. Every other Winnebago municipality has one. Re-checked 31 Jul 2026: still council seats only for both.",
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "county-precinct",
      "summary": "Henry County's voting precincts exist only as picture maps, one per township. No usable boundaries are published anywhere.",
      ...
      "wanted": "Henry County's precinct boundaries countywide as map data, plus polling places if published."
    },
    {
      ...
      "blocker": "All four routes are exhausted. The Clerk runs no elected-officials database; neither the Clerk nor the Election Information page links a yearbook or municipal directory, and the site's only directory is of county STAFF; the area's regional council publishes no member directory; and the county's municipal data carries names only, with no contact fields at all — unlike Lake's, which at least has hall address and phone.",
      "wanted": "Any Lee County list pairing a municipality with its mayor or president: a clerk's yearbook, a regional membership directory, or contact details added to the county's municipal data. Scraping 13 separate village websites is deliberately not the answer."
    },
    {
    ...
    },
    {
      ...
      "kind": "data-quality",
      "layer": "county-precinct",
      "summary": "Two of the five missing precinct polling places are now published; only Coloma 9's is still absent.",
      "blocker": "Re-checked 31 Jul 2026: the county's main polling place dataset still omits three buildings, so 55 of 60 precincts match — but two other county datasets now publish two of them with current 2026 primary details (Self Help Enterprises in Sterling, and Winning Wheels in Prophetstown), so combining them would reach 59 of 60. Coloma 9's polling place appears in none of the three.",
      "wanted": "Coloma 9's polling place on any published county dataset finishes this off. Combining the other two is already on our to-do list."
    },
    {
      ...
      "wanted": "Building IDs 43, 50 and 54 added to the county's polling place data, or any list pairing precincts with polling places. The other 76 precincts already name their site and address."
    },
    {
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "ward",
      "summary": "Whiteside publishes ward boundaries for six of its municipalities, but they date from before the redraw they would need to reflect.",
      "blocker": "The county's ward data carries 22 wards across Sterling (4), Rock Falls (4), Morrison (4), Fulton (5), Prophetstown (3) and Erie (2), named and complete. It was last edited on 5 Nov 2019 — BEFORE the 2020 census, and municipal wards are redrawn on that cycle. Showing it would answer “which ward am I in” with a pre-redistricting line and no way for a reader to tell. Rock Island's Moline and Silvis wards were used precisely because theirs were edited in 2022, after the redraw. Re-checked 31 Jul 2026: the date is unchanged, and no county or city statement about redistricting could be found.",
      "wanted": "An updated post-2020-census ward dataset, or confirmation from the county that these six municipalities did not redistrict. The boundaries are already published and would be shown the day they are known to be current."
    },
    {
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "county-precinct",
      "summary": "Ogle's voting precincts are not shown, even though the app already combines 2020 versions of them into the county's board districts.",
      "blocker": "The county publishes no precinct boundaries — only a 51-page PDF map book and a polling-location dataset that is points only. The 2020 Census versions ARE available and the board districts are built from them, but those are the 52 precincts of 2020 and the county now runs 51: it has since retired Forreston 3. That cannot move a board district line, because Forreston 1, 2 and 3 are all in District 7 and their combined shape is unchanged — but it would put a precinct on a card that no longer exists, and nothing published says where its territory went.",
      "wanted": "Ogle's CURRENT precinct boundaries as map data, or a county statement of how Forreston 3 was absorbed. The polling places are already published and would attach by name."
    },
    {
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "county-precinct",
      "summary": "Stephenson's voting precincts are not shown: the county publishes no current precinct boundaries.",
      "blocker": "The county redrew its precincts after the 2020 census — Freeport 1-18 became 01-16, and Waddams and West Point were renumbered — so census data cannot draw the current map. Found 31 Jul 2026: the state's precinct map archive does carry readable per-precinct files for the county, but at the 2021 pre-redraw state, so what is readable is the superseded map. The state's map viewer that might carry current data is still offline.",
      "wanted": "The county's current precinct boundaries as map data. The Freeport Township part of the same file would also replace the traced board district lines."
    },
    {
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "ward",
      "summary": "Rock Island City publishes its 7 wards with the current aldermen attached, but nothing proves the boundaries were redrawn after the 2020 census.",
      "blocker": "Checked 31 Jul 2026: the city's data carries the current council — all 7 confirmed against the city site and the county clerk's 2026 list — and was last edited in May 2025. But it was created in 2017 from a 2012 map, its only population figure is from 2010, and no redistricting ordinance from after 2020 could be found. The city code library that would carry one blocks automated access.",
      "wanted": "Confirmation of when the ward map dates from: a redistricting ordinance, a clerk's statement that the older wards were kept, or a republished map with 2020 populations. Everything else here is ready."
    },
    {
    ...
    },
    {
      ...
      "kind": "data-quality",
      "layer": "county-precinct",
      "summary": "Mason's 21 precincts match the 2020 Census records name for name, but nothing published confirms the county has not redrawn them since — so we do not publish them on that match alone.",
      "blocker": "Checked 2 Aug 2026, when the county was added: Mason publishes no precinct boundaries of its own, and the 21 precincts in 2020 Census data match the clerk's current list of 21 exactly by name. Matching names are not proof of matching lines — they say the names agree, not that no boundary moved — and showing 2020 lines as today's precincts would claim something the county has not. Menard and Cass need the same one-time confirmation.",
      "wanted": "Written confirmation from the County Clerk that the 21 precincts are unchanged since 2020 — or, better, the precinct boundaries as map data. We already have the shapes; what is missing is knowing they are current."
    },
    {
      "id": "mason-roster-is-a-scan",
      "concept": "County board members",
      "area": "Mason County",
      "counties": [
        "mason"
      ...
      "kind": "data-quality",
      "layer": "county-board",
      "summary": "Mason's board list is published only as a scanned image, so it is typed up by hand instead of refreshed automatically each week like every other county's.",
      "blocker": "Checked 2 Aug 2026, when the county was added: the roster is a photograph of a page. It does contain a hidden text layer, and that is the trap — the layer is encoded in a way that comes out as gibberish (“xRF# ISgH tlgP”) rather than failing outright. Gibberish that looks like text is worse than no text at all: automated reading would not report an error, it would publish nonsense under real people's names. So the list is typed up by hand, and a weekly check watches the two things software still can — that the county's page still links that same file, and that the file has not changed — and raises a flag for a person to re-read it.",
      "wanted": "The board list as real text: a web page, a text-based PDF, or a spreadsheet. Everything else about Mason updates automatically; this is the one list a person has to read."
    },
    {
    ...
    },
    {
      ...
      "kind": "data-quality",
      "layer": "county-board",
      "summary": "Macon publishes the right five board district shapes with every label blank — the districts are drawn, but nothing says which is which.",
      "blocker": "Checked 2 Aug 2026: the county's election map data holds exactly five district shapes created on 14 Sep 2022, so this is the current map — and the district number, representative, party, phone and email fields are empty on all five. The county's other board map holds only draft plans and the superseded seven-district version. Everything else is ready: the member list is published (15 members with party, district, phone and term), and the county's 64 precincts and its fire, library and park district boundaries could all be added today. This one missing label is what holds the board back.",
      "wanted": "District numbers for the five shapes — a labelled clerk's map, the adopted redistricting ordinance, or simply filling in the district field the data already has."
    },
    {
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "county-board",
      "summary": "Stark publishes all its geography in usable form — inside a Google My Maps — but at a date that predates the 2021 redistricting and cannot be verified.",
      "blocker": "Found 2 Aug 2026 by following the state's pointer file, which contains nothing but a Google My Maps link. Its export works and carries 51 shapes: 2 county board districts, 9 precincts, plus fire (6), school (4), library (6) and park (2) districts — an unusually complete set for a county of 5,300 people. The problem is the date. The state's pointer and companion files are from August 2020, BEFORE the late-2021 redistricting, and the county's online minutes only begin in July 2022, so the adopting resolution is not published anywhere reachable. The two districts of four members each do match the current 8-member roster, which the county publishes with email addresses and term ends.",
      "wanted": "The 2021 redistricting ordinance, or a re-export of the Google map at its current state. Either would add the board, the precincts and three district maps at once."
    },
    {
...
> | **The pass-7 build-ready ledger** (RESEARCH PASS 7, below) — ~~Peoria + Tazewell~~ **SHIPPED** (29th/30th), ~~Iroquois + Monroe + Randolph~~ **SHIPPED** (31st-33rd, the at-large posture's debut), ~~De Witt~~ **SHIPPED** (34th), ~~Washington~~ **SHIPPED** (35th), ~~Cass~~ **SHIPPED** (36th), ~~Marshall~~ **SHIPPED** (37th), ~~Mason~~ **SHIPPED** (38th) — the derivation tier is COMPLETE. **Champaign + Piatt WITHDRAWN — licensed, not open** (see the tranche-2 entry). STILL OPEN: the at-large tier — Pike, Putnam, Brown, Calhoun | nothing for the open tiers — every source measured live 2026-08-02 | **yes — the live work queue** |
...
- **RESEARCH PASS 7 (2026-08-02) — the frontier REOPENED: twenty-five counties now adjoin
  ...
  **Ford** (~13k — township-precinct composition with a SHARED split (Patton 3 sits in
  ...
  2 board districts, 9 precincts, PLUS fire 6 / school 4 / library 6 / park 2 — but the
  ISBE pointer dates to 2020-08 and the county's online minutes begin 2022-07, so the
  post-2021-reapportionment vintage is unconfirmable online), **Montgomery** (~28k — the
  ...
  **The pass-7 build-ready ledger** (the live work queue, recommended order):
  ...
  5. **The at-large tier** — Pike, Putnam, Brown, Calhoun (~31k), including Pike's
     permalink-gate widen.
  6. **Chases, not builds** (e-mail or artifact-hunt): Macon's district labels, Stark's
     2021 ordinance, Knox's rural map + roster access, Christian's roster + labeled map,
