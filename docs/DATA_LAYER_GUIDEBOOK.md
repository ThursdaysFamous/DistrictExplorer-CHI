...
```json
{
  ...
  "wisconsin": [
    ...
    },
    {
      ...
      "kind": "blocked",
      "layer": "ward",
      "summary": "The ward card names your ward but not your polling place — the state aggregates polling places only behind a lookup that refuses automated clients, so the card links that lookup instead.",
      ...
      "blocker": "MEASURED 2026-08-25. elections.wi.gov and myvote.wi.gov sit behind a Cloudflare managed JS challenge — HTTP 403 with `Cf-Mitigated: challenge` on every path tried, including /media/<id>/download file URLs and MyVote's own API routes (/api/*, /DesktopModules/GabMyVoteModules/api/*), to curl with full browser headers and to WebFetch alike. A challenge is an access control and is not defeated here. The last bulk statewide clerk/polling artifact visible in any search index predates 2020. ONE ROUTE REMAINS UNTRIED AND IS QUEUED: a CPD-style Playwright challenge ladder from GitHub Actions (this sandbox's Chromium has no egress, so it is unavailable rather than untried) — until that runs once, this block is recorded as measured-from-here, not permanent. A CITY-SCOPED OPENING EXISTS AND IS RECORDED: data.milwaukee.gov publishes \"Voting Wards and Polling Places\" (CKAN, CC-BY), which could pair Milwaukee's wards with buildings if a city-scoped enrichment is ever wanted; it does not answer the statewide question this gap records.",
