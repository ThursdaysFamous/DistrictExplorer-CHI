...
```json
{
  ...
  "wisconsin": ["us-house", "wi-senate", "wi-assembly", "wi-circuit-court", "wi-court-of-appeals", "county", "school-district-secondary", "school-district-unified", "school-district-elementary", "county-board", "aldermanic-district", "county-subdivision", "ward", "municipality", "zip-code", "school-site", "library", "police-station", "fire-station", "post-office"]
}
```
...
Fleet totals: **Chicago 39 · NYC 27 · SF 16 · Wisconsin 20** layers.
...
| Elected school board (districted) | SHIPPED `school-board` (ERSB) | NO HONEST ANALOG³ | NO HONEST ANALOG (at-large board)⁴ | NOT SHIPPED — Wisconsin school boards are mostly elected AT LARGE, and statute names the districted minority exactly (measured 2026-08-25): only MPS (ch. 119) and Racine Unified (s. 120.42(1)(d)2, nine election districts) elect by true geographic district. MPS is the recorded phase-3 city-scoped build (geometry on the city's own `election_geography` service layer 3, 8 districts + 1 at-large; roster on the MPS board page); RUSD is a measured gap — its geometry is published only as ArcMap-generated PDFs (a Jackson-style vector-path extraction or an ask to RUSD, never a raster trace). `docs/WI_PHASE2_PLAN.md` |
...
- **`mps-school-board`** — the districted-school-board build (ERSB precedent,
  city-scoped): geometry `election_geography/MapServer/3` via the CKAN SHP
  (milwaukeemaps drops ~1-in-4-8 requests — build-time only), roster the MPS board
  page (9 h3 headings, real DOM not inline-JSON). Blocker: SHP contents unopened
  (verify before building). RUSD stays a measured geometry gap (PDF-only).
