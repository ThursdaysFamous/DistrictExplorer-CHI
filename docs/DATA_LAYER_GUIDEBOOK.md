...
### A FIFTH and a SIXTH results vendor, and what each one can and cannot answer (2026-08-21)

Cumberland's join turned up `results.gbsvote.com`, and sweeping outward from it turned up
Clarity. Neither was in this repo. Together with the three already recorded they make five
platforms plus one county-run database, and the useful part is not the list — it is that
**each one answers a different question**, so knowing which vendor a county is on tells you
in advance whether that county can be built.
...
| **`results.gbsvote.com`** | **14** — Cass, Cumberland, Fulton, Greene, Grundy, Jasper, Johnson, Knox, Morgan, Perry, Scott, Warren, Washington (+ Parke, Indiana) | **SUMMARY canvasses only**: a per-contest precinct COUNT, plus one committeeperson contest per precinct |
