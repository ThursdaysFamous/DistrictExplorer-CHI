...
Fleet totals: **Chicago 39 · NYC 27 · SF 16 · Wisconsin 28** layers.
...
| Tax increment financing (TIF) district | SHIPPED `tif-district` (Cook, 418 — the Clerk's un-yeared current agency tiling, clerkTaxDistricts L18; dedicated Cook layer per the single-county rule until a second county ships — Kendall's `TIF_Districts` service is the recorded next entry) | n/a — New York State discontinued NYC-style TIF; no city program | n/a — SF uses IFDs/CFDs, no published district tiling evaluated | NOT SHIPPED — no statewide TID geometry (DOR publishes tabular TID data only; layers are per-municipality — Milwaukee's own CKAN TID dataset is the recorded city-scoped candidate) |
...
- ~~**Milwaukee city layers**~~ **SHIPPED 2026-08-26**, the third phase-3
  ...
  gate the fleet's other beat layers never had). The city TID dataset is
  the one remaining queued candidate from this entry.
