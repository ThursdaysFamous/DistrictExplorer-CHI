...
```json
{
  "chicago": [
    ...
    },
    {
      ...
      "kind": "data-quality",
      "layer": "municipality",
      "summary": "Wenona straddles the Marshall/LaSalle line and its two county clerks publish different city councils — one names eight members, the other six.",
      "blocker": "Found 3 Aug 2026, when Marshall's municipal source shipped. Both clerks publish Wenona in full, and both agree on the mayor (Mary Jane Bade) and the clerk. They do not agree on the council. LaSalle's directory names EIGHT alderpersons — Flanigan, John Simmons, Julia Kitchens, Kym Healy, Nathen Anderson, Randy Lohr, Russell Skaggs Sr. and William Simmons — and no treasurer. Marshall's table names SIX trustees — Anderson, Healy, Zulz, John Simmons, Flanigan and Skaggs — plus Treasurer Jaclyn DeRubeis, and dates every seat 2027 or 2029. So Kitchens, Lohr and William Simmons appear only in LaSalle's list, Matt Zulz only in Marshall's, and the two even spell Anderson (Nathan/Nathen) and Longbons (Deborah/Deb) differently. The card currently ships LaSalle's, which is what it shipped before Marshall was added — deliberately NOT switched, because choosing would mean dropping three named people on our own reading of which document is fresher. Marshall's is the stronger-looking source on its face (it is headed as the 2025 consolidated-election result and carries a term year per seat, which is the tie-break that settled Dwight), but that is an inference, and officeholder data is the one place this project does not act on inference.",
      "wanted": "Either clerk confirming Wenona's current council, or the city itself publishing it. That single answer decides which of the two rosters is right and retires this."
    },
    {
      ...
      "kind": "data-quality",
      "layer": "county-precinct",
      "summary": "All 60 of the county's precincts now name a polling place — but two of the buildings come from the Clerk directly, because the county's own list still omits them.",
      "blocker": "CLOSED for the reader, still open at the source, 3 Aug 2026. The join is 56 of 60 — re-measured live, and the previous entry was WRONG about which precinct: Coloma 9 resolves fine (facility 4, Rock River Christian Center), the Clerk confirmed that venue by e-mail the same day, and the layer already agreed with her, so an override written for it was reverted rather than ship dead code for data that is already correct. The four that do not resolve are Sterling 9, 14 and 18 (all facility 22) and Prophetstown 1 (facility 26), neither id being among the 29 voting locations the county publishes. Asked which buildings those are, County Clerk Karen Stralow named them: facility 22 is Self Help Enterprises, facility 26 is Winning Wheels. Those two records now ship in data/app/whiteside-precinct-polling.json and are consulted ONLY where the county's own layer has no match, so the day the county publishes them the file stops being read with no code change. The card says on those four precincts that the building came from the Clerk rather than the published list, and shows no address, because she gave names and an address found elsewhere would not be hers. She has said she is forwarding the two locations to the county's GIS department.",
      "wanted": "The two missing voting locations added to the county's own published polling list, which would retire our two hand-entered records — and their street addresses, which are the one thing those four cards still cannot show."
    },
    {
