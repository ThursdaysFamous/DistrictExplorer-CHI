...
> | **The pass-7 build-ready ledger** (RESEARCH PASS 7, below) — ~~Peoria + Tazewell~~ **SHIPPED** (29th/30th), ~~Iroquois + Monroe + Randolph~~ **SHIPPED** (31st-33rd, the at-large posture's debut), ~~De Witt~~ **SHIPPED** (34th), ~~Washington~~ **SHIPPED** (35th). **Champaign + Piatt WITHDRAWN — licensed, not open** (see the tranche-2 entry). STILL OPEN: Marshall, Mason and Cass (the PDF-sourced derivation remainder) and the at-large tier — Pike, Putnam, Brown, Calhoun | nothing for the open tiers — every source measured live 2026-08-02 | **yes — the live work queue** |
