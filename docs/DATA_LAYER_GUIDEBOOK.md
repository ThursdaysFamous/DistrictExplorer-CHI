...
```json
{
  "chicago": [
      ...
      },
    {
      ...
      "kind": "no-source",
      "layer": "county-board",
      "summary": "Alexander County's three commissioners aren't shown. The county names two of them; nobody names the third.",
    ...
    },
    {
      ...
      "kind": "data-quality",
      "layer": "ward",
      "summary": "Galesburg's seven council wards and 20 precincts are published, current, and still not shown here — because Knox County isn't covered yet.",
      "why": "Coverage is added a whole county at a time, and no county-wide Knox boundary has a source yet, so a Galesburg ward would sit inside a greyed-out county.",
      "blocker": "Found 3 Aug 2026 in the pass-11 probe. The City of Galesburg runs a 75-service ArcGIS Online account which publishes Galesburg_City_Council_Wards, a Precincts layer (20 city precincts carrying ward, county board district and polling place), and Knox County Board Districts *in the City of Galesburg*. All three are real and current. None is countywide, so none of them serves Knox County itself. That is what makes this a decision rather than a build: every one of the 21 ward groups shipped today sits in a county inside the coverage ring, 21 of 21 with no exception, and Galesburg would be the first outside it. A Galesburg resident would then resolve the ward layer — which is dispatched, not statewide — while the out-of-scope wash greys their location out, which is the shape of the 2026-07-30 Kankakee bug rather than the Centralia municipality case (see scripts/build_metro_outline.py: `municipality` is statewide, `ward` is not). Adding Knox to the ring to compensate would be worse: nothing county-keyed answers anywhere else in Knox. DECIDED 4 Aug 2026, in the same decision that retired contiguity as a shipping gate (EXPANSION_GUIDE §2.5.1): the county stays the unit of coverage — a city cannot carry its unserved county in, so these wards wait for Knox rather than shipping into the wash. Note the retirement itself changes nothing here: Knox borders Fulton, so it was never contiguity-blocked; this gap always turned on the county-keyed test, and still does.  RE-MEASURED 2026-08-21, AND THE CITY PUBLISHES MORE THAN THIS RECORD KNEW. Galesburg's ArcGIS Online org (services1.arcgis.com/T5ar9pn3YeFZ47Wh) serves TWO public feature layers, both reachable and current, linked from the city's own \"Galesburg Wards and Precincts\" lookup app at gis.ci.galesburg.il.us/Wards. (1) Galesburg City Council Wards: seven features carrying WARD, POPULATION and MEMBER — the sitting council member's NAME on the boundary, which is the roster half this project usually has to scrape separately, and the seven populations run 4,029 to 4,092, a 1.6-point spread. (2) Galesburg Voting Precincts: twenty features carrying PRECINCT, WARD, POLL and a COUNTY field the city's own web map labels \"County Board District\". THE SECOND LAYER IS THE MORE IMPORTANT FIND and it belongs to Knox's record rather than this one: the city is a SEPARATE ELECTION AUTHORITY, the Galesburg Board of Election Commissioners, and its precincts carry Knox County Board Districts 1, 2 and 3, grouping 7 / 7 / 6. That is why the County Clerk's canvass never mentions those three districts, and it withdraws the split-precinct conclusion recorded against Knox. THE DECISION HERE IS UNCHANGED: a city cannot carry its unserved county into the ring, so all of this still waits on Knox — but what it waits on is now smaller and better named, and the ward layer will ship with a member on every boundary the day the county joins.",
      "wanted": "Any county-wide Knox boundary — board districts or precincts from the County Clerk — brings the county in, and Galesburg's wards ship with it."
    },
    {
    ...
    },
    {
        ...
        "kind": "no-source",
        "layer": "county-board",
        "summary": "Knox County's five board districts aren't drawn, so the card can't say which one you're in — and the county's member list sits behind a site that refuses automated visits.",
        "why": "The county's website turns away automated visits, and its GIS server publishes townships and parcels but no precinct or board-district layer. Nothing published says which townships districts 4 and 5 are made of.",
        "blocker": "Checked 2 Aug 2026: knoxcountyil.gov — and the old address, which redirects into the same block — refuses every request, including the page that lists all 15 members with district, term and contact details. The only usable district boundaries published anywhere are on the City of Galesburg's map account: districts 1 to 3, the three that fall within the city, adopted by the County Board on 27 Oct 2021. Districts 4 and 5, the rural remainder, appear in no usable source we could find, and the state's countywide map is provably 2011 content, from before the 2021 redistricting. BALLOT ROUTE CHECKED AND CLOSED 2026-08-20: il-knox.pollresults.net returns the vendor's generic 7,583-byte shell with 4 electionData blocks — the response a made-up county name gets — against the ~52 blocks a carried county returns, so no certified per-precinct return is available to compose districts from. Knox is on neither the GBS nor the platinumelectionresults county list either. That closes the route that settled Franklin and Wayne, and leaves this record's own asks (the 2021 board packet, or a mirror the blocking does not cover) as the only ones open. ENCLOSED 2026-08-21: Warren's join made Knox the coverage wash's THIRD ENCLAVE, after Bureau and Christian. Every one of its seven neighbours — Henry, Stark, Peoria, Fulton, McDonough, Mercer and now Warren — is served, so this county now reads as a doughnut on the map rather than as frontier, which makes its absence the visible kind. Nothing about the blocker changed: the countywide 2021 map is still most likely inside the 27 Oct 2021 board packet on a site that refuses automated visits. What changed is who notices. A FOURTH RESULTS VENDOR, found 2026-08-21 and recorded here because nobody had it: results.gbsvote.com (GBS). It is not the accessliberty/pollresults pair and not platinumelectionresults.com; it carries THIRTEEN Illinois counties — Cass, Cumberland, Fulton, Greene, Grundy, Jasper, Johnson, Knox, Morgan, Perry, Scott, Warren and Washington — of which FIVE are unserved (Cumberland, Jasper, Johnson, Knox, Perry). Each county page lists its election authority and an archive of result sets back to 2016 at /locations/county_results.asp?id=N, and it was reached from the county's own Elections page rather than by guessing a hostname. FOR KNOX IT OPENS A DOOR AND THEN SHOWS THE ROOM IS PROBABLY EMPTY. The vendor carries Knox with an archive back to 2016 and names its election authority (County Clerk Scott G. Erickson), which is a route into a county whose own website refuses automated visits — worth recording for that alone. But the 2024 General reports COUNTY BOARD DIST 4 across 13 precincts and DIST 5 across 15, against a county total of 29. Two of five districts covering 28 of 29 precincts is only possible if precincts are SHARED between districts, which would put Knox with Bureau, Cumberland, Douglas, Ford and Piatt in the split-precinct class that no whole-precinct dissolve can draw. That reading is consistent with what this record already says — only the Galesburg half of the five districts exists as usable map data, the city being where the lines cut. IT IS NOW MEASURED SHUT RATHER THAN SUGGESTIVE, 2026-08-21, on the vendor's own archive across SIX result sets and TWO districting plans. Under the 2021 map: Nov 2022 (OFFICIAL) DIST 4 = 13 and DIST 5 = 15 against a county total of 28 — the districts covering the whole county; Mar 2024 (OFFICIAL) the same 13 and 15 against 28; Nov 2024 13 and 15 against 29; Mar 2026 (OFFICIAL) 13 and 15 against 29. Under the 2011 map: Nov 2018 15 and 16 against 31, Nov 2020 (OFFICIAL) 15 and 16 against 32. In every one of them TWO of the five districts report across the entire county bar at most one precinct, which leaves nothing for districts 1, 2 and 3 — and a whole-precinct plan requires the five districts' precinct counts to SUM to the county total, not for two of them to reach it alone. So Knox's precincts are split among its board districts, under both plans, and no whole-precinct dissolve can ever draw them. Knox joins Bureau, Cumberland, Douglas, Ford, Jackson and Piatt in the split-precinct class. THE PRECINCT LISTS THEMSELVES REMAIN UNAVAILABLE and that is a fact about the vendor, not the county: GBS publishes countywide totals with an \"N of N precincts reporting\" counter and no per-precinct breakdown anywhere, so the earlier note that this rested on COUNTS rather than LISTS cannot be resolved through this route at all — what replaced the missing lists is arithmetic repeated across six elections rather than one. The earlier remark that \"the 2022 page renders a different structure and could not be compared\" was wrong: it parses like the others.  KNOX HAS TWO ELECTION AUTHORITIES, MEASURED 2026-08-21, AND THAT SINGLE FACT OVERTURNS THIS RECORD'S CENTRAL INFERENCE. The paragraph above argues that because County Board Districts 4 and 5 report 13 and 15 precincts — reaching the County Clerk's own total of 28 without leaving room for Districts 1, 2 and 3 — Knox's precincts must be SPLIT between districts, and files the county alongside Bureau, Cumberland, Douglas, Ford, Jackson and Piatt. THAT ARGUMENT ASSUMES ONE AUTHORITY AND KNOX HAS TWO. The CITY OF GALESBURG runs its own BOARD OF ELECTION COMMISSIONERS, which canvasses the city's precincts in a separate document the County Clerk's canvass does not contain — so the Clerk's \"28 of 28\" is the county OUTSIDE Galesburg, not the county. Districts 4 and 5 partition that remainder; Districts 1, 2 and 3 lie inside the city. Nothing here is split, and nothing here is proven unsplit either: the split-precinct conclusion is WITHDRAWN as unestablished rather than replaced with the opposite claim. THE CITY PUBLISHES ITS HALF, AS LIVE PUBLIC DATA. Galesburg's ArcGIS Online org carries \"Galesburg Voting Precincts\" (services1.arcgis.com/T5ar9pn3YeFZ47Wh, FeatureServer/0), 20 precincts, each with PRECINCT, WARD, POLL — and a field named COUNTY which the publisher's OWN web map labels \"County Board District\", valued 1, 2 and 3 and grouping the twenty precincts 7 / 7 / 6. Its item description states the precincts were \"developed by the Galesburg Board of Election Commissioners and approved on January 4, 2022\". SO BOARD DISTRICTS 1, 2 AND 3 ARE DRAWABLE TODAY, from the city's own current data, with no census dissolve and nothing inferred. Districts 4 and 5 are not: nothing published says WHICH of the Clerk's precincts belong to which, and no geometry for the Clerk's precincts exists anywhere this project can read. THE CENSUS ROUTE IS SHUT FOR THE COUNTY HALF, and this is measured rather than assumed. TIGER's Census 2020 layer gives Knox 52 voting districts summing to its exact 49,967 — GALESBURG township plus GALESBURG CITY 1 through 21, and 30 others. The Clerk's own canvass headers track a fabric that has MOVED since: 31 precincts in the 2020 primary, 32 in the 2020 general, 31 in the 2021 consolidated, 28 from 2022 through the 2024 primary, \"28 OF 29\" in the 2024 general and 29 in the 2026 primary. The city moved too, 21 census precincts to the 20 it publishes. A county whose fabric moved is a county the Jasper test fails, so no dissolve of census geometry can answer for the Clerk's side — while the city's side needs no census at all, because Galesburg publishes the polygons. ONE THING THE 2026 PRIMARY ADDS AND ONE IT LEAVES OPEN. It NAMES 28 of the Clerk's precincts, one committeeperson contest apiece per party: Cedar First through Fourth, Chestnut, Copley, Elba, Galesburg, Haw Creek, Henderson, Indian Point, Knox First through Knox Six, Lynn, Maquon, Ontario, Orange, Persifer, Rio Twp, Salem, Sparta, Truro, Victoria and Walnut Grove. Its header counts 29. The twenty-ninth is not named anywhere in it, and it is not a seventh Knox-township precinct — KNOX SEVENTH appears in no committeeperson contest in any election on this vendor back to 2020, where First through Six appear repeatedly. Which precinct the twenty-ninth is, and why Districts 4 and 5 still count 13 and 15 against a county of 29, are open questions and are recorded as open.  THE COUNTY HAS A PUBLIC GIS SERVER AND THIS RECORD SAID IT DID NOT, 2026-08-22. The line above — \"knoxcountyil.gov, and the old address which redirects into the same block, refuses every request\" — is true of the county's WEBSITE and was written as though it described the domain. gis.knoxcountyil.gov is a self-hosted ArcGIS Server, public, answering 200, and it serves THE COLES PATTERN: leaf certificate without its GoDaddy intermediate, so every automated client reports \"unable to get local issuer certificate\" and no browser notices. Supplying the AIA intermediate with its hash PINNED (sha256 973a41276ffd01e027a2aad49e34c37846d3e976ff6a620b6712e33832041aa6) reaches it normally; verification is never disabled. It was found from a URL handed to this project — its Parcel Viewer web app, whose AGOL item names the service — and NOT by the standing sweep, which probed only the bare and www clerk-mail domains. scripts/probe_incomplete_tls_chains.py now also probes `gis.<domain>` for exactly this reason; a county's boundary data lives on its GIS host far more often than on its www host. WHAT THE SERVER CARRIES, enumerated rather than hoped: three folders and sixteen services, all of them parcels, aerials, sales, terrain and the Regional Office of Education. Public/Knox_V4/MapServer holds 56 layers including TOWNSHIP (20 polygons, one clean name apiece) and CORP POLY (16 municipalities, Galesburg among them). THERE IS NO PRECINCT LAYER AND NO COUNTY BOARD DISTRICT LAYER, on the server or in the county's public ArcGIS Online org (40 public items, swept: aerials, parcels, sales, road closures, ROE 33 — nothing electoral). So the Clerk's precinct geometry is still absent, and that is now MEASURED ON THE COUNTY'S OWN GIS rather than inferred from a blocked website. BUT THE TOWNSHIPS CHANGE THE ASK, and this is the part worth acting on. The Clerk's 28 precinct names — the ones the 2026 primary's committeeperson contests list — map onto those 20 townships one for one, with exactly two townships subdivided: Cedar carries Cedar First through Fourth, Knox carries Knox First through Knox Six, and the remaining 18 precincts each match a whole township by name (18 + 4 + 6 = 28, the Clerk's own count). Districts 4 and 5 tabulate 13 and 15 precincts. If each district is a union of whole townships — with Cedar's four and Knox's six each sitting wholly inside one district — then District 4 and District 5 can be drawn TODAY from this server: union the townships, subtract the Galesburg city polygon from Corp Poly (the city is its own election authority and is where districts 1, 2 and 3 live). The arithmetic is consistent with that shape (6 + 7 = 13 and 4 + 11 = 15 both partition the 18 single-precinct townships) and does NOT establish it, which is the difference between a route and a build. THE ONE MISSING FACT is which townships are in which district — a single sentence from the Clerk, not a shapefile — and the held draft now asks for exactly that instead of for 'boundaries in whatever form exists'.  AND THE ROSTER IS NO LONGER UNREACHABLE EITHER, 2026-08-22, from a THIRD Knox host nobody here had tried. This record says the county's page listing all 15 members \"with district, term and contact details\" is behind the block, which is true of knoxcountyil.gov and says nothing about where that site's DOCUMENTS live: they are served from its Revize CMS at cms2.revize.com/revize/knoxcounty/, which answers 200 to this client with no cache-buster needed. The June 24, 2026 board minutes open with a roll call that names EVERY SEAT BY DISTRICT — District 1 Tracy Robertson, Cheryl Nache, Anthony Weiss; District 2 Erin Pugh, Robert Bondi, Jennifer Fredrick; District 3 Kimberly Thierry, Pamela Davidson, Samuel Cohen; District 4 Joshua Kramer, Jared Hawkinson, Todd Olinger; District 5 Ricardo 'Rick' Sandoval, Patty Boone, Brian Friedrich — with Hawkinson named as Chair. Fifteen seats, three per district over five districts, which is the structure this record already had from the returns. THAT IS A THIRD READABLE HOST FOR ONE COUNTY: the website denies, the GIS server serves the Coles pattern, and the CMS serves the documents outright. A COUNTY IS NOT BLOCKED BECAUSE ITS WEBSITE IS — that rule was already written here for election authorities (Johnson and Perry shipped without their counties' sites being read at all) and this is its document-host case. WHAT THE CMS DOES NOT CARRY: its archive under that naming holds only recent minutes (June, August and September 2025; May and June 2026) — no 2021 anywhere, so the 27 Oct 2021 redistricting packet is still out of reach, and guessed names for a map, ordinance or precinct list all 404 with no directory listing. KNOX STILL DOES NOT SHIP, and the reason is now precisely one thing rather than two: districts 4 and 5 have no geometry. Shipping districts 1-3 alone was considered and rejected — the layer would appear inside Galesburg and vanish in the rest of the county, which reads to a rural resident as 'no county board here' rather than as a gap, and that is worse than an honest absence.",
        "wanted": "Which townships — or which of the Clerk's precincts — make up County Board Districts 4 and 5. The county's own GIS server publishes the 20 township polygons and the Galesburg city boundary, so the composition is the only missing piece."
      },
    {
