...
```json
{
  "chicago": [
      ...
      },
    {
      ...
      "kind": "no-source",
      "layer": "county-board",
      "summary": "Alexander County's card names two of its three commissioners. Nobody names the third, and the card says a seat is unlisted rather than leaving you to count.",
      "why": "The county's board page lists two commissioners, and its published minutes stop in spring 2024, so no current source names the third seat.",
      "blocker": "Probed 4 Aug 2026 in the pass-13 detached-counties sweep: no self-hosted ArcGIS under ten hostname patterns across two service roots, and the ArcGIS Online catalogue names nothing county-keyed for it. No county website answered under the five domain patterns probed. Whether the board is districted or elected county-wide was not determinable in this pass — determine it from a certified election document (EXPANSION_GUIDE §2.5 step 2) before any build: an at-large answer makes this a roster ask (the tranche-5 County-card path), not a geometry ask.  NOT YET ASKED: this records what the pass-13 probe could see, which Ogle proved on 2026-08-03 is a different question from what the county will send on request — its precinct shapefile arrived by return e-mail from a gap that read exactly like this one. See \"The ask ledger\". CORRECTED 2026-08-09 by a web search, the step this record was written without. The line above claiming no county website answered is FALSE. alexandercounty.illinois.gov answers 200 and is the county's own site, with a County Board page naming Chairman Joe Griggs and Vice Chairman Bruce Sims; alexandercountyil.org, the Clerk's e-mail domain, also resolves. The systematic failure: the probe permuted the county's NAME, while the answer was already in this repo — data/app/il-county-clerks.json carries each Clerk's e-mail, and for 9 of the 14 counties recorded this way the CLERK'S E-MAIL DOMAIN IS THE COUNTY'S WEB DOMAIN. This project was e-mailing these counties at those very domains on 2026-08-05 while telling readers the counties had no website. RE-MEASURED 2026-08-21, AND THE ROSTER IS THE BLOCKER RATHER THAN THE FORM. Two corrections to the line above. First, alexandercountyil.org — the Clerk's e-mail domain — does NOT serve a site from here: it resolves to 208.91.197.27 and refuses TLS outright (tlsv1 alert internal error). Alexander is therefore the ONE frontier county the clerk-domain rule does not reach, and the pattern that does reach it is the state's own hosting, alexandercounty.illinois.gov (169 KB) — a sweep of that pattern across all 28 frontier counties is recorded separately and adds no other county. Second, the board page no longer says what this record says it says: it now names exactly TWO members, Chairman Joe Griggs and Member James Smith. Bruce Sims, whom this record named on 2026-08-09, is gone — his /team/bruce-sims/ page returns 404, as does /team/larry-essex/, and both survive only as stale entries in the site's navigation menu, which is why a link scrape sees four names and the page itself shows two. ISBE's county-board structure table gives Alexander as THREE commissioners elected at large. So the county publishes two of three, and the third is neither named nor explained anywhere — vacancy, resignation or simple omission, nothing published says which, and guessing is exactly what the honesty rules forbid. THE FORM IS STILL UNCONFIRMED BY A CERTIFIED DOCUMENT: the structure table is 2007-vintage and Richland proved on the same day that it can be wrong, both election-results vendors return their generic shells for Alexander, and ISBE's precinct archive carries only statewide offices. ONE GOOD SIGN FOR LATER: that archive lists Alexander's ten precincts — CACHE, CAIRO 1-5, MCCLURE, OLIVE BRANCH, TAMMS, THEBES — with NO sub-precinct reporting units at all, the opposite of Richland's nine, so nothing yet suggests a split fabric here.  THE FORM IS SETTLED 2026-08-21, from the county's own official document rather than from the 2007 table. The county publishes its board minutes as .docx files on the County Board page, and every one of them carries the board's composition in its letterhead. The most recent, 7 May 2024, reads \"ALEXANDER COUNTY BOARD OF COMMISSIONERS ... Chairman: Joe Griggs / Vice Chairman: Bruce Sims / Commissioner: L. ‘Steve’ Essex\", and its roll call names those three and nobody else. That is the county describing its own board: THREE MEMBERS, commission form. A SECOND, INDEPENDENT MARKER AGREES and costs nothing to check: TIGER's county-subdivision layer gives Alexander SEVEN subdivisions, every one coded LSADC 29 (precinct) rather than 44 (township), which is the census marker for a county NOT under township organization — the same shape Massac has, and Massac ships as a three-commissioner County-card county. Read that marker as CORROBORATION and never as proof on its own: it establishes non-township organization, not a seat count and not at-large election, and Gallatin is the counter-example that proves the difference — a TOWNSHIP county whose five-member board is elected countywide all the same. WHAT REMAINS IS ONE NAME. The board has three seats and the county's page publishes two, Chairman Joe Griggs and Member James Smith. The two who sat with Griggs in May 2024 are both gone — Bruce Sims and Larry ‘Steve’ Essex — and their /team/ pages 404 while surviving only as stale navigation entries. The minutes STOP at 7 May 2024, so the county's own record of who serves ends more than two years back and cannot name today's third commissioner; James Smith appears in no minutes at all. Alexander is therefore NOT shipped, and the reason is worth stating because it is not the usual one: nothing here has to be guessed to ship two members, but shipping a three-seat board with two names would conceal a seat rather than invent one, which is the same harm from the other direction.  SHIPPED ANYWAY ON 2026-08-23, AND THE REASONING ABOVE IS WHAT CHANGED RATHER THAN THE FACTS. Alexander joins as the 83rd county served, through the County card alone. Everything the paragraph above measured still holds — the county publishes two of three, its minutes stop at 7 May 2024, and the third seat is named nowhere — and the sentence that kept it out (\"shipping a three-seat board with two names would conceal a seat\") turned out to describe a card this project does not have to build. The districted counties have said \"1 of 3 seats not listed in the county's directory\" on their cards since Livingston; the County card simply had no way to say it, because no at-large county had ever published fewer members than it seats. It does now: the roster block carries `seats` beside its members, and the shortfall renders in the same words. A card that names two commissioners and states that a third is unlisted conceals nothing. A NEW CERTIFIED WITNESS ARRIVED WITH IT, and it settles the form from the county's own returns rather than from the 2007 structure table. platinumelectionresults.com DOES carry Alexander — county id 46 — which this record's line about \"both election-results vendors return their generic shells\" got wrong by testing the pollresults/accessliberty pair and stopping. Its certified 2024 General canvass carries \"ALEXANDER COUNTY | FOR COMMISSIONER\" reporting ALL ELEVEN of the county's precincts, the same denominator the presidential contest on that ballot reports — a countywide board contest, which is the same gate build_from_returns applies to Union and Williamson. It also corroborates the roster exactly: JAMES R. SMITH beat LARRY S ESSEX 1,082 to 1,078 for the one seat on that ballot, which is the county page's James Smith and explains why Essex's /team/ page 404s. THE VENDOR CARRIES 2024 AND NOTHING ELSE for this county — every other slug answers with the vendor's 395 KB redirect rather than a report — so the elections that seated Chairman Griggs and the unnamed third commissioner are not published anywhere this project can read, and the returns cannot fill the seat either. THE PARSER READS THE WIDGET AND IGNORES THE MENU, which is the trap this county sets for the next maintainer: every page on the site carries a hand-built nav listing \"County Board Chairman / County Board Member / County Board Vice Chairman\" pointing at /team/joe-griggs/, /team/larry-essex/ and /team/bruce-sims/. The last two are 404s of people out of office, and a scrape of /team/ links anywhere on the page would ship FOUR commissioners for a three-seat board, two of them wrong. The Elementor team widget on the County Board page is the county's maintained list; the menu is decoration that outlived its subjects. CONTACT SHIPS AND ADDRESSES DO NOT: each member's own /team/ page publishes a personal e-mail and a mobile number, which is the county publishing a way to reach the office, and the only address anywhere on those pages is the courthouse in the site footer, read once from a constant rather than per-person (the Madison/Peoria rule). THE ASK IS OUT. The third-attempt e-mail to Clerk Price was SENT on 2026-08-22 at 22:28 UTC, onto the same thread as the 5 and 16 August asks — verified in the sent mail rather than assumed, which is the whole of the Scott lesson working in the other direction: a date belongs here when the message LEFT, and a sent ask must not go on being described as held. It asks the one question this record has always asked, and answering it turns a two-name card with a stated gap into a three-name card. Nothing had come back as of 2026-08-23.",
      "wanted": "The name of the third commissioner, or word that the seat is vacant. That single fact is the whole ask."
    },
    {
    ...
    },
    {
      ...
      "kind": "no-source",
      "layer": "county-board",
      "summary": "Wayne County's seven board districts aren't drawn, so the card can't say which one you're in. All 14 members are named with their district.",
      "why": "The county publishes which precincts make up each district, and those groupings come out so unevenly populated that something in them looks wrong — so nothing is drawn until the county confirms them.",
      "blocker": "MEASURED AGAIN 2026-08-20 AND ONE HALF OF THE ASK IS ANSWERED. Probed 4 Aug 2026 in the pass-13 detached-counties sweep: no self-hosted ArcGIS under ten hostname patterns across two service roots, and the ArcGIS Online catalogue names nothing county-keyed for it. The county site (waynecountyil.gov) is real: a Wayne County Board page, a Voting and Elections page, and an Online Mapping link into Sidwell's Portico portal — an ArcGIS-based PARCEL product whose configuration loads client-side; no election layer surfaced. waynecounty.org is a domain-for-sale page, a decoy. Whether the board is districted or elected county-wide was not determinable in this pass — determine it from a certified election document (EXPANSION_GUIDE §2.5 step 2) before any build: an at-large answer makes this a roster ask (the tranche-5 County-card path), not a geometry ask.  NOT YET ASKED: this records what the pass-13 probe could see, which Ogle proved on 2026-08-03 is a different question from what the county will send on request — its precinct shapefile arrived by return e-mail from a gap that read exactly like this one. See \"The ask ledger\". ANSWERED 2026-08-20 WITHOUT ASKING, from two sources that corroborate each other and disagree in a way worth recording. FORM SETTLED — DISTRICTED, seven districts of TWO members each. The county's own Wayne County Board page publishes the composition outright: DISTRICT 1 Berry, Garden Hill, Keith, Orchard, Indian Prairie, Hickory Hill (Daryl Hargrave Chairman, Quinton Greenwalt); 2 Mt. Erie, Bedford, Elm River, Massillon, Zif (Randy Hedrick, Greg Keyser); 3 Lamard 1-2, Jasper 1-2 (Brandon Bittles, Eddie Barbre); 4 Arrington, Four Mile, Orel (Cody Ehrhart, Steve Manning); 5 Big Mound 1-2, Barnhill (Matt Shreve, T.J. Vaughan); 6 Grover, Rider (Vern Hutson Sr., Bill Bruce); 7 Merriam, Golden Gate (Gene Kollak Vice Chairman, Steve Troyer). INDEPENDENTLY DERIVED FROM THE BALLOTS, via platinumelectionresults.com (Wayne is county/14 — see the third-vendor entry): all 27 precinct pages for the 2024 General were fetched and grouped by which board candidate each precinct actually voted on. They partition all 27 exactly once, into seven groups. SIX OF THE SEVEN MATCH THE COUNTY'S PAGE PRECISELY. THE SEVENTH IS THE FINDING: the county's page lists seven districts totalling only 25 precincts, and the two it omits — FAIRFIELD 1 and FAIRFIELD 2 — voted in District 7's contest on the certified 2024 ballot (Steve Troyer's, alongside Merriam and Golden Gate). So the page is incomplete rather than the ballots being wrong, and 25 against the county's 27 is the arithmetic that says so. District 5's group is the one that returned NO CANDIDATE in 2024 — its seat was on the ballot with nobody filed, which is why the ballot route names six people and the page names fourteen. THE FABRIC PASSES: Census 2020 carries exactly 27 Wayne voting districts summing to the county's exact 16,179, and all 27 current precinct names map onto them (Massilon/Massillon and Goldengate/Golden Gate are spelling variants between the two sources, not different places). WHY THIS IS NOT YET A BUILD, and the reason is population rather than provenance. Composed against a 2,311 ideal the districts run D1 2,572 (+11.3%), D2 1,895 (-18.0%), D3 3,060 (+32.4%), D4 2,229 (-3.6%), D5 2,387 (+3.3%), D6 1,863 (-19.4%), D7 2,173 (-6.0%) — a spread of about 52 points, and D3 EXCEEDS THE 30% CEILING this repo's own Mercer builder uses. That ceiling exists to catch a mis-assignment, and here it fires on a district whose composition BOTH sources agree on exactly, so the likely reading is that the county's plan really is this unbalanced rather than that the derivation slipped. That is a fact about Wayne and not a defect to smooth: a builder must either establish why (a plan drawn on older figures, or one never redrawn after 2020) and accept it on the record the way Mercer's -14.6% was accepted, or find the county's own adopted map. Nothing ships until that is stated rather than assumed. CORRECTED 2026-08-21: the line above calling waynecountyil.gov a domain-for-sale decoy is FALSE. It answers 200 with 231 KB and is titled \"Home - Wayne County, Illinois\" — the county's own site. It was found by the frontier-wide sweep of nine domain patterns that also found Jasper's, and it was missed before because the earlier probe read the domain once and recorded what it saw that day. NOTHING ABOUT THE MEASURED BLOCKER CHANGES: Wayne's composition is already known from its certified 2026 primary and the districts it confirms run 1,863 to 3,060 against a 2,311 ideal — a 32.4% worst deviation, past the 30% ceiling this project's own dissolve guard enforces — so the county still cannot be built from that composition. What the working site adds is a place to look for the adopted plan itself, which is the one thing that would settle whether the composition or the guard is wrong.  THE SITE THAT TURNED OUT TO EXIST NOW CARRIES THE PLAN (2026-08-21, hours after the domain sweep found it). waynecountyil.gov/wayne-county-board/ states the whole composition in plain text — \"representatives from 7 districts, covering 20 townships\", fourteen members two per district, each district followed by its precincts: D1 Berry, Garden Hill, Keith, Orchard, Indian Prairie, Hickory Hill; D2 Mt. Erie, Bedford, Elm River, Massillon, Zif; D3 Lamard 1, Lamard 2, Jasper 1, Jasper 2; D4 Arrington, Four Mile, Orel; D5 Big Mound 1, Big Mound 2, Barnhill; D6 Grover, Rider; D7 Merriam, Golden Gate. That is a far better source than the canvass this record was resting on, and it AGREES WITH IT. THE FABRIC MATCHES 27/27. Wayne's census voting districts number 27 and sum to its exact 16,179, and every name on the county's list is one of them once two SPELLINGS are aliased — the county writes MT. ERIE and MASSILLON where the census writes MT ERIE and MASSILON. No re-precincting, nothing left over. THE PAGE OMITS ITS OWN COUNTY SEAT, which is the one thing it does not settle: FAIRFIELD 1 and FAIRFIELD 2 (487 + 724 = 1,211 people) appear in no district list on it. The county's certified 2026 primary answers that — a numbered District 7 contest ran on both Fairfield ballots — so the two sources together are complete, and it is worth naming which one supplies which fact rather than blurring them. AND THE BALANCE IS NOW CONFIRMED BY TWO INDEPENDENT SOURCES rather than inferred from one. Under the county's own published composition the districts run: D1 2,572 (+11.3%), D2 1,895 (-18.0%), D3 3,060 (+32.4%), D4 2,229 (-3.6%), D5 2,387 (+3.3%), D6 1,863 (-19.4%), D7 2,173 (-6.0%) — summing to the county exactly, a 51.8-point total spread against a 2,311 ideal. THE 32.4% IS NOT A MIS-ASSIGNMENT, which is what this project's 30% dissolve ceiling exists to catch; it is what the county's own plan says. So the ceiling is now the only thing between Wayne and a build. THE OPERATOR WAS ASKED AND CHOSE NEITHER SHIPPING NOR HOLDING: the 51.8-point spread is to be treated as EVIDENCE that something neither source reveals may be wrong — a split precinct, a stale page, an unrecorded amendment — and the county is to confirm its own seven district lists before anything is built on them. A reply is DRAFTED into the existing thread with Clerk Woodrow (the 5 Aug ask and its 16 Aug follow-up, both unanswered) putting two questions: whether the list is current and complete as the Board is elected today, and whether Fairfield sits in District 7 as the certified primary indicates. It says plainly why it asks rather than simply using the page — that the spread looks wider than an adopted plan's, which more likely means a misreading than an unusual plan. NOTHING HAS BEEN SENT and no ASKED date is recorded here; the draft awaits review.",
      "wanted": "Confirmation from the county that its seven district lists are current and complete, and that Fairfield sits in District 7. An adopted map would answer it all at once."
    },
    {
