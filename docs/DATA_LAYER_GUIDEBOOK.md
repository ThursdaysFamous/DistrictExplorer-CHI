...
```json
{
  "chicago": [
    ...
    },
    {
      ...
      "blocker": "Checked 2 Aug 2026: the county's online mapping publishes 28 datasets, including a 34-precinct map created in April 2026 that matches the clerk's own polling table exactly, and a township map — but no board districts, confirmed by going through all 28. The districts exist as “NEW-2022-Clinton-County-Board-Districts.pdf”, a readable map drawn over the county's 15 townships. The member list is published in full: all 15 with phone, email and term. RE-MEASURED 2026-08-20, and both halves hold with one thing worth naming. THE PRECINCT LAYER IS REAL AND IS THE COUNTY'S OWN: services3.arcgis.com/gnvxvf9aoRBjW15Q (reached through the county's Sidwell-hosted portal, the same vendor Cumberland uses) carries `voting_dist_ClintonCo` with 34 polygons, and all 34 of its precinct names appear on the Clerk's current Precinct Polling Places page — 34/34. The org was enumerated in full at 29 services and still carries NO board-district layer, so the original finding stands against a bigger catalogue than it was written against. THE TRAP IN THAT LAYER, recorded because its two obvious signals both mislead: its fields are the CENSUS 2010 VTD schema (STATEFP10, VTDST10, GEOID10, NAME10) and its lastEditDate is April 2026. Neither is the data's vintage. The GEOID10 column is BLANK on every feature, so it is not a census file being passed off as one; and an upload date is not a survey date (the Coles lesson). What actually establishes currency is the 34/34 agreement with the Clerk's polling page, and that is the check to repeat rather than the edit date. THE CENSUS ROUTE WOULD NOT WORK HERE, which is worth knowing before anyone reaches for it: Census 2020 gives Clinton 39 VTDs against the county's 34, summing to the county's exact 36,899. The gap is CONSOLIDATION, not a moved fabric — Irishtown 1+2, Lake 1+2 and St Rose 1+2 each became one precinct, and six more differ only by the space in LOOKINGGLASS vs LOOKING GLASS — but Brookside went 5 to 3, and no reading of the names says which census district went into which county precinct. So the Calhoun composed-dissolve route is closed by one township. It does not matter, because the county publishes the geometry itself. SO THE PRECINCTS ARE BUILDABLE TODAY, from the county's own layer, the way Morgan's were — Clinton could join for precincts alone without its board districts ever being resolved.",
    ...
    },
    {
      ...
      "blocker": "The clerk's directory at macoupinvotes.gov builds its list after the page loads, so the page itself arrives empty and the address it pulls names from is assembled on the fly rather than written in the page. The county's downloadable export (dataset rxtc-9j2k, 892 rows including aldermen by ward) was last updated on 3 Nov 2015; publishing that today would present a decade-old snapshot as the current officeholders. INVESTIGATED 2026-08-20 AND THE RECORD'S CLAIM IS EXACT, but its proposed fix cannot be performed from this environment and the reason is worth recording. THE PAGE REALLY IS EMPTY, measured rather than assumed: macoupinvotes.gov/election-results/elected-officials/ returns 78,839 bytes containing ZERO occurrences of Mayor, President, Alderman, Alderperson, Trustee, Ward, 'Village of' or 'City of'. WHAT THE COMPONENT IS: a vendor WordPress plugin, soe_elected_officials_3.0.0 (the same SOE suite that supplies the site's precinct finder and voter-eligibility widgets). Its own scripts.js is 1,522 bytes of pure styling — it measures a link colour and a table cell's padding — and carries no endpoint. Neither do the SOE framework scripts (soe_init_3.0.js, soe_component_utils_3.0.js): the only URL in either is a Google Maps link. The page localises admin-ajax.php exactly once, as AjaxURL for the EVENT CALENDAR plugin, not for this component, and NO soe_elected_officials container renders in the body at all. Five plausible admin-ajax action names (soe_elected_officials, get_elected_officials, elected_officials, soe_get_elected_officials, soe_elected_officials_get) each return WordPress's unregistered-action '0' with HTTP 400. WHY THE DEVTOOLS STEP CANNOT BE DONE HERE: this project's sandbox runs Chromium that does not use the agent HTTPS proxy, so the headless browser cannot reach external sites at all (it is why the smoke test serves Leaflet from a local vendor directory). Capturing the component's XHR therefore needs a human with an ordinary browser, or a CI runner, and is not a gap in the research so much as a gap in what this environment can observe. The wanted line stands exactly as written — one look in devtools — and now says who has to take that look.",
    ...
    },
    {
      ...
      "wanted": "Not the form, not the composition and not the roster — all three are established above from the county's own page and its certified ballots. TWO THINGS: confirmation that Fairfield 1 and Fairfield 2 belong to District 7 (the ballots say so; the county's page omits them), and an explanation of the population spread — whether the plan predates the 2020 census or was drawn on other figures. Either the county's adopted map or a sentence from the Clerk would settle both."
    },
    {
