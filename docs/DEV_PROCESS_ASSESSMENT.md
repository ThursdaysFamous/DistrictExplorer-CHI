...
- **R6 (part 1) — SHIPPED (2026-08-24): the districtry skin became engine, so it stops being
  ...
  as **fork-local** text. Measured before deciding: every skin line in `il/index.html` sat
  outside every fence, so the only way to rebrand the siblings was to copy it into them. That is
  "the redesign done twice", which is the failure the consolidation exists to end and which the
  retired `build_districtry_preview.py` had already committed once.
  **The whole CSS half turned out to be one element.** `<style id="districtry-skin">` holds 986
  interior lines — the masthead, the fleet switch, the chrome tints, and the entire dark-mode
  block — with no GENERATED or TEMPLATE marker inside it and **not one Chicago literal**, which
  was checked rather than assumed. It fences as a single ENGINE block.
  **The JS half fences as one block that deliberately starts late.** The theme controller and
  `buildMetroSwitch()` are equally metro-agnostic, but the section they live in opens with the
  GENERATED `dark-map-palette` region — a table of one metro's own layer colours. An ENGINE
  fence may not contain a GENERATED marker, and it should not want to: the controller READS that
  table and never owns it, so the fence starts on the line after it.
  **Nothing changed for anyone.** The diff to `il/index.html` is **28 lines added, zero
  removed** — two fence markers and their rationale. `ny/` and `ca/` are untouched to the byte,
  because `compose_app.py` splices a block only into an instance that CARRIES its fence: an
  instance opts in, and one that has not opted in keeps its old skin exactly. That is what makes
  stage 2 a safe flip rather than a rewrite, and it is why this stage ships on its own.
  **Two things stay per-instance and that is correct, not a shortfall:** the self-hosted
  `@font-face` block (its own `SELF-HOSTED FONTS` fence vocabulary, generated by
  `build_fonts.py` into each instance's own `fonts/`) and the dark map palette above. Fonts are
  bytes on disk per app; layer colours are facts about one metro's layers.

- **R5 (part 3) — SHIPPED (2026-08-24): the sibling-metro switcher stopped sending readers to
  retired domains, by running the sync that already existed.** Eight of the nine
  `METRO_EXPLORERS` urls across the three apps still pointed at `chidistricts.com`,
  `nyc.chidistricts.com` and `sf.chidistricts.com` — including SF's own entry, which offered a
  reader standing on `/ca/` a handoff back to `/ca/` through a Squarespace forward. They
  "worked" only because those forwards are up, which is why no gate and no reader complaint
  surfaced them: a 301 to the right page is indistinguishable from a correct link until the
  forward lapses.
  **Nothing was hand-edited, and the fix is the finding.** `METRO_EXPLORERS` has been GENERATED
  since Conversion 3 — each app's `metro-config` region is emitted from its worksheet's
  `metro_explorers`, and `generate_metro_files.py --sync-fleet` projects `metros.json` into
  those worksheets. `metros.json` has carried districtry.com paths since R5 part 1. **The whole
  defect was that the sync was never run**, so the manifest and the apps disagreed for as long
  as nobody asked. Two commands closed it.
  **What that says about the R5 checklist:** changing a url in `metros.json` is not a change
  until `--sync-fleet` and a regenerate follow it, and no gate can say so — `--check` compares
  each app against ITS OWN worksheet, which was itself stale, so all three passed. The manifest
  comment now states the coupling in the place someone will read it before making the next
  change.
  **Two references were deliberately NOT touched.** The Jo Daviess licence text in
  `metro-worksheet.json` and in the shipped boundary file records that display on
  `chidistricts.com` was authorized in writing by the county's IT/GIS Director under Digital
  Data License Agreement #008382. That is a quotation of what a licensor actually granted, not
  a link to maintain, and editing it would misstate the licence. **It does raise a real
  question for a human rather than a build:** the data now displays on districtry.com, which is
  not the domain named in the grant, so the county should be told the address changed. The
  schema files' `$id` (`https://chidistricts.com/schema/metro-worksheet.schema.json`) is also
  left alone — a `$id` is an identity, not a location, and changing one is a schema change
  rather than a domain fix.
...
- **R5 (part 2) — SHIPPED (2026-08-24): one privacy page for the whole site, generated by
  ...
  **`il/privacy.html` is a redirect shell, not a deletion.** #493 landed the same day with
  root-level shells for four Illinois pages that moved DOWN into `/il/` at R2.3; this page moved
  UP, and gets the mirror image for the same reason. It was Illinois-specific for about a day —
  short-lived is not the same as unlinked, it was in `sitemap.xml`, and a crawler that fetched
  it or a reader who kept it should land on the page that answers rather than on a 404. Same
  mechanism as #493's: meta refresh, canonical naming the real page, JS replace, and
  deliberately not `noindex`, which would block the consolidation.
