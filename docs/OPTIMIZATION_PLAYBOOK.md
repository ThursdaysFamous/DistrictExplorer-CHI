...
|---|------|--------|--------|------|
| R2-1 | **Kill render-blocking (~2,110 ms):** ~~fonts non-render-blocking (~780 ms)~~ ✅ ~~+ inline `leaflet.css` (~750 ms)~~ ✅ (~1,530 ms shipped); async-load `leaflet.js` (~1,340 ms) remains | **High — every visit** | Low → Med | PSI |
