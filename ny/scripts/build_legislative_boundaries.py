...
Why this exists: the U.S. House / NY State Senate / NY State Assembly layers used
to call loadTigerLayer(idx), which fetches every New York district for that
chamber from tigerweb.geo.census.gov in one shot — ~1-2 MB gzip each. The CHI
fork measured this at ~5.7 s per first toggle in a production Firefox profile;
this is the R2-2 pattern back-ported from DistrictExplorer-CHI. Legislative
districts change only on the decade (post-census redistricting), the exact
profile the anchor boundaries were externalized under. This script does the same
for the legislative geometry: fetch → simplify → validate → write
data/app/<chamber>-districts.json, which index.html then fetches like any other
data/app boundary. A ~5.7 s live query becomes a ~200 ms same-origin fetch.
...
officeholder ROSTERS (congress-roster / ny-senate-members / ny-assembly-members)
...
LAYERS = {
    ...
    "state-senate": {
        ...
        "out": "state-senate-districts.json",
        "simplify": "10%",
        "min_features": 63,  # 63 NY State Senate districts
    },
    "state-assembly": {
        "layer": 2,
        "fields": ["SLDL", "NAME", "BASENAME", "GEOID", "STATE"],
        "out": "state-assembly-districts.json",
        "simplify": "8%",
        "min_features": 150,  # 150 NY State Assembly districts
    },
}
...
def fetch_tiger(layer, fields):
    """Fetch every New York feature for a Legislative MapServer layer as GeoJSON.

    Uses curl so it works through an HTTPS proxy (as in the Claude Code sandbox)
    and anywhere curl is present. STATE='36' returns all NY districts for the
    chamber in one query (no transfer-cap paging for these layers)."""
    url = (
        TIGERWEB + "/" + str(layer) + "/query"
        "?where=" + "STATE%3D%27" + NY_FIPS + "%27"
