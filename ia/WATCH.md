...
| Mid-decade — Iowa specifically | County-level SUPERVISOR districts move off the census cycle under Iowa's own law: **Senate File 75** (signed 2025-04-11) forced Story, Johnson and Black Hawk counties from at-large to district elections for November 2026, and litigation over the resulting maps is active as of this writing — see `docs/IA_EXPANSION_PLAN.md`'s thesis. This does not affect the four layers shipped in this PR (chambers + county are all TIGERweb, not county-supervisor), but it is the first thing to re-check when `county-supervisor` ships | _(watching — relevant to the not-yet-shipped county-supervisor layer)_ |
| Same cycle | County subdivisions / places / school districts shift in TIGERweb continuously; when those layers ship (later phase-1 PRs) their PRE-BUILT files need the same rebuild discipline as the chambers | _(not yet shipped)_ |
