...
ROSTER_FILES = {
    ...
    "ia-city-officials.json": 5,  # The elected officials of the FIVE Iowa cities whose own pages a machine can read — a mayor and five at-large council members apiece, 30 people with an e-mail each. NOT a statewide roster and never to be described as one: a sweep of all 532 Iowa cities that publish a website found 16 with a readable roster and 5 that cleared every check, against 407 cities publishing no site at all, so `ia-municipal-officeholders` stays open for the other 847 -- these five, the three cities that elect by ward, and the 84 named by their own COUNTY in ia-county-city-officials.json are the 92 that name anybody -- and the card says so on every one of them. All five are AT-LARGE, so they are roster rows on the City card and not a layer. Every address is re-tested each build against the name actually shipped by the same rule build_ia_county_officers.py applies — the officeholder's own name in the local part, or an office mailbox — because six of the 30 sit on consumer, provider or business domains that these small cities publish as official contact. Built by ia/scripts/ia_city_officials_scraper.py + build_ia_city_officials.py, refreshed weekly by update-ia-city-officials-roster.yml.
    "ia-county-city-officials.json": 80,  # The mayors, clerks and council members EIGHT IOWA COUNTIES publish for the 84 cities inside them — 605 officials, every one of the 84 naming a mayor, 74 naming a clerk, and 93 carrying the ward or at-large seat their own county states, which no other Iowa source pairs with a name at all. THE 605 ARE NOT ALL COUNCIL: 421 council members, 84 mayors and 75 clerks, plus 25 who are none of those — 8 city administrators, 5 park commissioners, 5 park board, 5 library trustees, a city manager and an administrative secretary — shipped under the role their own county publishes, which is why the card's block is headed CITY OFFICIALS rather than claiming an election. TWO MARKUP FACTS ARE PER-COUNTY AND NOT UNIVERSAL, and an earlier version of this note said otherwise: five of the eleven put the clerk's and mayor's role in a <b> inside the name's own div while six give every role a positionTitle heading — and the <b> is an ADDITION rather than an alternative, since all five still take City Council from a heading, so what it carries is the NON-COUNCIL roles (Crawford's third is Library, Boone's and Winnebago's City Administrator), and the mailto href is EMPTY in Boone and Crawford, POPULATED in Cerro Gordo, Shelby and Marion, and absent in the other six — the scraper reads the link TEXT and the role structurally, so it needs neither to be universal. This is the route `ia-municipal-officeholders`'s own blocker had recorded as NOT YET PROBED: the county auditors, Iowa's statutory commissioners of elections under Iowa Code 47.2. Eleven counties publish such a page and three are REFUSED by a dated gate rather than by a hand list — Sac, Shelby and Winnebago still publish terms that ended in January 2026, and all three show their four-year seats split across 2025 and 2027 where the other eight straddle 2027 and 2029, the signature of a page last maintained after the November 2023 city election — so a county that updates its page ships on the next weekly run with nothing to edit here, and one that goes stale drops out the same way. NOT a statewide roster: 847 of Iowa's 939 cities still name nobody and the City card says so on every one of them. Built by ia/scripts/ia_county_city_officials_scraper.py + build_ia_county_city_officials.py, refreshed weekly by update-ia-county-city-officials-roster.yml.
