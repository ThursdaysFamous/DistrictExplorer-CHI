#!/usr/bin/env python3
"""
Build the pre-simplified Iowa county boundary file in data/app/ from Census
...
answers anywhere in the state, so the county layer ships whole — fetch ->
...
Usage:
    python3 ia/scripts/build_state_counties.py
"""
...
SIMPLIFY = "10%"
MIN_FEATURES = 99  # every Iowa county — no water pseudo-district on this layer
...
def fetch_tiger():
    """Fetch every Iowa county feature as GeoJSON. Uses curl so it works
    ...
    if not feats:
        raise RuntimeError("TIGERweb State_County layer returned no Iowa features")
...
def main():
    ...
    if n != MIN_FEATURES:
        raise RuntimeError(
            ...
            "Iowa counties) — refusing to write" % (n, MIN_FEATURES)
        )
