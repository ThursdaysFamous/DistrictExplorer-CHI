...
ROSTER_FILES = {
    ...
    "boone-district-officials.json": 4,  # The office and board of Boone County's five park and library taxing bodies, keyed by the same district name the two parcel-dissolve boundary files carry — the card joins the two, and scripts/build_boone_district_officials.py REFUSES TO WRITE unless the roster's keys are exactly the districts drawn, because a name drifting apart would empty a contact block silently while the polygon still drew. THE CLERK'S YEARBOOK OWNS THE OFFICE AND DOES NOT OWN EVERY BOARD, which is the correction this build made to its own gap record. The booklet supplies the address, telephone, fax, website and agency head for all five, and the four commissioners of the Belvidere Park District, which publishes no board page of its own. It is SUPERSEDED for Ida Public Library: the library's own board page names a different board and states the mechanism — an annual officer election on 28 July 2026, three weeks after the 2026 edition's own 7 July date — so Ida's eight trustees and its ONE OPEN SEAT come from the library, and the disagreement is printed every run rather than reconciled away. A vacancy is the thing an annual snapshot can never show, which is why the snapshot's completeness is evidence against it (the mi-commissioner-roster reasoning). Rockford Park District's five commissioners, Cherry Valley's seven trustees and North Suburban's seven likewise come from those bodies' own publications, which the booklet does not carry at all. CHERRY VALLEY WAS NEARLY SHIPPED AS 'no board published' OFF ONE HTTP 502 — it publishes seven trustees with a term year and an individual address each, so the scraper now retries a 5xx before believing it. Four Belvidere Park staff (a park attorney and three superintendents) parse cleanly and are dropped on purpose: they are employees, not the governing body. Ida's director address ships as the library's own mindyl@ rather than the yearbook's mistyped mindlyl@, the one place a second publisher settles which spelling is right.
}
