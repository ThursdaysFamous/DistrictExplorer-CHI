...
ROSTER_FILES = {
    ...
    "grundy-county-board-districts.json": 2,  # Grundy County Board districts (3 six-member districts) DERIVED by scripts/build_grundy_board_districts.py — the county's own precinct layer dissolved per the adopted 'Approved County Board Districts (10/12/2021)' map (the county GIS publishes no board geometry). The precinct-to-district transcription is proven by the map's own printed populations: all three district totals reproduce to the person (17,663 / 17,364 / 17,506 = 52,533). Regenerate when the county re-precincts or reapportions; --check is the drift gate.
