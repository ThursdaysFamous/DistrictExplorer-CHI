...
GEOMETRY_FILES = {
    ...
    "belvidere-city-outline.json": (1, 1),  # City of Belvidere corporate limits (scripts/build_place_outline.py) — the ONLY municipal outline this instance ships, and it exists to carve one hole in a county source rather than to draw a city. Boone's Fire_Districts layer tiles every acre of the county, so a Belvidere address returned Fire District 2 while the Clerk's own district-rates report gives 18 of 55 tax codes no fire line at all — those 18 being the city's, which runs a MUNICIPAL fire department. TWENTY-TWO of its twenty-four rings are HOLES, unincorporated pockets the city annexed around, and they are load-bearing: a resident of one is not in the city, so they ARE in the fire district and keep their card. The even-odd test both the builder and pointInGeometry use gets that right, and the builder gates its own area against the Census's published AREALAND so a hole counted as solid (43.05 km² against a real 35.19) fails the build.
    ...
    "boone-library-districts.json": (3, 3),  # Boone's 3 library districts, pre-built by scripts/build_parcel_fabric_districts.py. Boone publishes NO library boundary — measured across its whole ArcGIS server (12 folders, 99 services) and its AGOL org (510 items) — so each district is the DISSOLVE of the parcels the county's own fabric assigns to the tax codes its Clerk's 2025 Taxcode Value within District Report puts in LYBV (18), LYCV (2) and LYNS (4), road voids closed at 75 ft by the same rules as the Rock Island files. Two of the three are seated in Winnebago and reach across the line; only their Boone slice is drawn. Bounds are an EQUALITY: the county levies exactly these three, and shipping only Ida would tell a Loves Park or Cherry Valley resident they are in no library district.
    "boone-park-districts.json": (2, 2),  # Boone's 2 park districts, pre-built by scripts/build_parcel_fabric_districts.py from the parcels the county's own fabric assigns to the tax codes its Clerk's 2025 Taxcode Value within District Report puts in PDBV (25) and PDRF (1) — the county publishes no park boundary of any kind. Same 75 ft road-void closing and probe gates as its library sibling; Rockford Park District is seated in Winnebago and only its Boone slice is drawn. Bounds are an EQUALITY: the county levies exactly these two, and the rest of Boone genuinely sits in no park district.
    "woodford-fire-districts.json": (17, 17),  # Woodford's 17 fire protection districts, pre-built by scripts/build_parcel_fabric_districts.py. The county publishes no district tiling — its “Fire Protection Districts” service is its 25,824-row PARCEL FABRIC, one record per property — so each district is the DISSOLVE of the parcels whose Fire_Prote column names it, road voids closed at 75 ft by the Rock Island rules. The set is corroborated by the Clerk's certified 2025 Tax Year 1st Distribution settlement sheets, which levy for exactly these 17, code for code. Bounds are an EQUALITY. The Village of Metamora is a deliberate hole: all 1,701 parcels with no fire district are inside it, it runs its own department, and the district around it is named Metamora RURAL.
    "woodford-library-districts.json": (6, 6),  # Woodford's 6 library districts, pre-built by scripts/build_parcel_fabric_districts.py from the Library_Di column of the same 25,824-row parcel fabric, with the same 75 ft closing and probe gates as its fire sibling; the Clerk's certified 2025 settlement sheets levy for exactly these 6. Bounds are an EQUALITY. One of the six, Minonk City Library, is MUNICIPAL rather than a district — its 1,230 parcels are the City of Minonk exactly, and none lie outside it — so its feature carries a note the card renders.
    "woodford-park-districts.json": (3, 3),  # Woodford's 3 park districts, pre-built by scripts/build_parcel_fabric_districts.py from the Park_Distr column of the same 25,824-row parcel fabric; the Clerk's certified 2025 settlement sheets levy for exactly these 3. Bounds are an EQUALITY: most of the county genuinely sits in no park district, and only 4,113 of its parcels are in one.
    ...
    "calhoun-county-outline.json": (1, 1),  # Calhoun County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. Calhoun elects its five commissioners at large, so its MEMBERS ride the County card and there is no board geometry; its precincts dispatch. This note used to say index.html names no loader for the file and that it was fetched by slug alone — true when it was written (#290), stale since #398 gave the county a dispatch entry and a loadCalhounCountyOutline of its own, so the dynamic_reference exemption is dropped and the reference check now covers it.
    ...
    "carroll-library-districts.json": (7, 7),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py. NOT a county or library publication, and it CARRIES A LICENCE rather than nothing: AGOL item 675906ae06b5460296154760b7fb1367 states public use permitted, no warranty, NOT for legal boundary determinations, an ATTRIBUTION requirement (users must cite the source) and a MODIFICATION-DISCLOSURE requirement (users may modify the data but must indicate any changes when sharing derived products). This file IS modified — clipped to the county and simplified at 5 m — so the card discloses both the publisher and those changes. Only LAYER 11's sub-metadata is empty; the service and the item are not. Carroll's 7, and the county that validates the whole source: these are exactly the seven library tax lines the County Clerk's own Tax Code by District Listing names, and the builder refuses to write if they ever stop matching. Shannon and Lake Carroll correctly sit in NO library district, which the Clerk's tax codes independently confirm.
    ...
    "cass-precincts.json": (21, 21),  # Cass County's twenty-one voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 21/21 with NO ALIAS and they sum to its exact 2020 population of 13,042. Bounds are an EQUALITY because twenty-one is the county's whole precinct list, named by its certified 2026 General Primary one committeeperson contest at a time (forty-two contests, both parties) under a header reading "21 OF 21 PRECINCTS REPORTING". NOTHING WAS EVER BLOCKING THIS COUNTY: it has been served since its board districts shipped and simply had no gap record saying its precincts were absent, which is how a county goes unbuilt while every guard stays green — the 2026-08-20 audit that found eleven such counties is why it exists. No board district is carried here (cass-county-board-districts.json ships those four separately, and the same canvass reports their contests over 4 + 5 + 6 + 6 precincts, summing to the county's exact twenty-one). No polling place ships (scripts/build_cass_precincts.py).
    ...
    "clay-county-outline.json": (1, 1),  # Clay County coverage outline — the coverage test for the county's board dispatch entry (clayCountyCoverage). Shipped 2026-08-04 as gap-location geometry by the pass-13 detached-counties sweep, when Clay read as source-less; promoted to a coverage outline on 2026-08-26 when the county joined as the 91st served county, so index.html now names it literally and it is no longer a dynamic reference.
    "clay-county-board-districts.json": (14, 14),  # Clay County's 14 LETTERED County Board districts (A-N), one member each, composed from Census 2020 voting districts per the composition the county's own board page states letter by letter — independently witnessed for seven letters by the county's certified 2026 General Primary, which ran single-member contests lettered B, C, D, G, I, K and N with no precinct in two districts. THE COUNTY SEAT IS SPLIT AND THE CLERK DREW THE LINE: the census carries ONE Clay City voting district (1,166 people) where the county runs districts A and B through it, and nothing published said where that line falls until Clerk Amy Britton answered directly (e-mail, 2026-08-24): District A is 'within the Village limits of Clay City', District B 'the unincorporated area of Clay City/Stanford'. That is a corporate-limits line the census draws exactly — intersected at the BLOCK level, 108 blocks matching the voting district's 1,166 people, EVERY block sorting cleanly to one side (none straddles), 833 in the village (A) and 333 outside (B, joining Stanford's 625). One measured margin rides the note rather than being absorbed: two blocks (14 people) inside the village limits lie in the STANFORD voting district and ship in District B with their precinct. WHY IT SAT PAST THE SPLIT ANSWER: population. District J (Harter V, one whole voting district) runs +39.8% against the 949 ideal and District L (Harter III) +33.3% — past the fleet's usual 30% ceiling and its largest accepted deviation — shipped only after the Clerk confirmed the plan itself (e-mail, 2026-08-26): 'These are the current maps.' The ceiling is raised for Clay alone with the measured value recorded (scripts/build_clay_boundaries.py; --check is the drift gate). NO PRECINCT FILE ships: the county's board page names 19 precinct-slots (Clay City twice) while ISBE's certified 2026 returns list 18 (one Clay City), and shipping either count would overstate what is known.
    ...
    "cumberland-county-outline.json": (1, 1),  # Cumberland County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. It shipped on 2026-08-04 as gap-location geometry only, from the pass-13 detached-counties sweep, and was promoted on 2026-08-21 when Cumberland joined ON ITS PRECINCTS: its three compass-point board districts split precincts and are permanently shut to a whole-precinct dissolve, so the precincts dispatch alone and no board district ships.
    "cumberland-precincts.json": (12, 12),  # Cumberland County's twelve voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 12/12 against the county's own precinct names and they sum to its exact 2020 population of 10,450. Bounds are an EQUALITY because twelve is the county's whole precinct list, counted by its own certified canvass ("OF 12 PRECINCTS REPORTING", and every countywide contest on it) and named by that canvass one contest at a time. NO ALIAS IS NEEDED and the reason is worth keeping: this project's own record predicted one, having read the Clerk's polling notice, which groups the precincts as "NEOGA I & II", "SUMPTER I & II" and "GREENUP I, II, & III". Those are POLLING-PLACE GROUPINGS — three buildings serving seven precincts — not precinct names; the certified returns print GREENUP 1/2/3, NEOGA 1/2 and SUMPTER 1/2 in arabic exactly as the census does. NO BOARD DISTRICT ships and none ever will from this route: the county's own returns report its board contests as Central 6, Eastern 5 and Western 3 precincts, fourteen against a county of twelve, so two precincts sit in two districts each. NO POLLING PLACE ships — the county publishes a list, and that belongs with a roster guard and a date (scripts/build_cumberland_precincts.py).
    ...
    "douglas-county-outline.json": (1, 1),  # Douglas County coverage outline — the coverage test for the county's board and precinct dispatch entries (douglasCountyCoverage). Shipped 2026-08-04 as gap-location geometry by the pass-13 detached-counties sweep; promoted to a coverage outline on 2026-08-23 when the county joined as the 88th served county, so index.html now names it literally and it is no longer a dynamic reference.
    ...
    "warren-precincts.json": (26, 26),  # Warren County's 26 voting precincts, named exactly as the county's own precinct-map LEGEND prints them. Census 2020 drew 28 voting districts here, so TWO census pairs are merged back to the county's fabric — Roseville 1 + Roseville 2 into Roseville, and Spring Grove-Alexis + Spring Grove-Gerlaw into Spring Grove — and each merged feature names the census units it is built from as censusUnits, so a reader can still find it in census products. Both merges are NAMEABLE and each sits wholly inside a single district, which the builder re-proves every run rather than trusting a comment; that is the same fact that lets Warren's districts be dissolved from census geometry at all, which is why nothing new had to be sourced for this file. The absence simply had no gap record explaining it (the Cass/Greene/Scott/Moultrie shape). Every precinct carries its County Board district. No polling place ships (the Calhoun rule; scripts/build_warren_boundaries.py).
    "knox-county-board-districts.json": (5, 5),  # Knox County's 5 County Board districts, and the only county in the fleet whose districts come from TWO publishers at once. Districts 1, 2 and 3 lie inside the CITY OF GALESBURG, which runs its own Board of Election Commissioners, and are taken exactly as the city publishes them. Districts 4 and 5 are unions of WHOLE TOWNSHIPS from the county's own GIS server — a host whose record said it 'refuses every request', which was true of the county's website and false of gis.knoxcountyil.gov, a public ArcGIS Server serving the COLES PATTERN — minus the city area. The composition is transcribed from the county's own adopted map, 'Knox County Board Districts 2022' (Knox County GIS Department, 01/28/2022), on which the black-and-yellow DASHED line is the district boundary and the thick ORANGE line the precinct boundary. THREE WITNESSES AGREE: the map names the townships; the county's certified returns report exactly 13 precincts in District 4 and 15 in District 5, in every election on the vendor archive back to 2022, without ever naming one; and Census 2020 populations land within 6.3% of the per-district ideal, districts 4 and 5 within 0.7%. A COLOUR-SAMPLING READ OF THE SAME MAP WAS TRIED FIRST AND WAS WRONG — its fills are per-township decoration, not per-district — and was caught by the precinct-count gate rather than by inspection, which is why that arithmetic is a gate in the builder and not a comment (scripts/build_knox_board_districts.py).
    ...
    "gallatin-county-outline.json": (1, 1),  # Gallatin County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. It shipped on 2026-08-04 as gap-location geometry only, from the pass-13 detached-counties sweep, and was promoted on 2026-08-21 when Gallatin joined: the board is elected countywide, so its five members ride the COUNTY card while the precincts dispatch.
    "gallatin-precincts.json": (11, 11),  # Gallatin County's eleven voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 11/11 against the county's own precinct names and the districts sum to its exact 2020 population of 4,946. Bounds are an EQUALITY because eleven is the county's whole precinct list, counted by its own certified canvass ("PRECINCTS COUNTED (OF 11)"). Corroborated twice more: the Clerk's precinct map is a legible plate labelling all eleven, and precinct committeeperson contests across the 2024 and 2026 primaries name nine of them with nothing outside the set. THE COUNTY WAS ONLY READABLE AT ALL once the COLES PATTERN was found a second time — gallatinco.illinois.gov serves an incomplete TLS chain, so every automated client called it dark while the site rendered fine in a browser. NO BOARD DISTRICT ships and none ever will: Gallatin elects its five board members countywide ("CO.BD.MEMBER CWD" on its own ballots), so they ride the County card. NO POLLING PLACE ships — the county publishes a list, and that belongs with a roster guard and a date (scripts/build_gallatin_precincts.py).
    "greene-county-outline.json": (1, 1),  # Greene County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. It shipped as gap-location geometry, served the County card when Greene's seven at-large board members joined on 2026-08-08, and was promoted on 2026-08-21 when its twenty-two precincts dispatched.
    "greene-precincts.json": (22, 22),  # Greene County's twenty-two voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 22/22 and they sum to its exact 2020 population of 11,985. Bounds are an EQUALITY because twenty-two is the county's whole precinct list, named by its certified 2026 General Primary one committeeperson contest at a time under a header reading "22 OF 22 PRECINCTS REPORTING". ONE ALIAS is applied and it is a DESIGNATION rather than a spelling: the county's returns write WRIGHTS 2 where the census leaves WRIGHTS unnumbered, so the alias renames the census feature onto the county's own designation — never a merge, which apply_aliases refuses. NO BOARD DISTRICT ships and none ever will: the same canvass carries a single countywide COUNTY BOARD contest per party over all twenty-two precincts, so Greene elects its board county-wide and its seven members ride the County card. No polling place ships (scripts/build_greene_precincts.py).
    ...
    "hardin-county-outline.json": (1, 1),  # Hardin County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. It shipped on 2026-08-04 as gap-location geometry only, from the pass-13 detached-counties sweep, and was promoted on 2026-08-23 when Hardin joined ON ITS PRECINCTS: its board is elected countywide, so there is no district geometry to seek, and its roster stays unshipped because the county's own web domain is a parking lander on both hosts.
    ...
    "jackson-county-outline.json": (1, 1),  # Jackson County coverage outline — the coverage test for the county's board and precinct dispatch entries (jacksonCountyCoverage). Shipped 2026-08-03 as gap-location geometry by the pass-10 frontier sweep; promoted to a coverage outline on 2026-08-23 when the county joined as the 87th served county, so index.html now names it literally and it is no longer a dynamic reference.
    ...
    "jo-daviess-precincts.json": (28, 28),  # Jo Daviess County's 28 voting precincts, dissolved from the 29 Census 2020 voting districts by scripts/build_jodaviess_precincts.py. The ONE difference from the census fabric is Warren, merged from Warren I and Warren II — a self-describing merge witnessed CURRENT by two county offices: the Clerk's certified 17 March 2026 General Primary results, which name the 28 in prose with Warren singular, and the GIS department's polling map of 12 Aug 2026, which writes "Dunleith I, II, III" where precincts share a building and so would have written "Warren I, II" had there been two. The 29 districts sum to the county's exact 22,035, which proves the fabric tiled the county in 2020 and NOT that it is current — that comes from the two witnesses. Bounds are an EQUALITY. Not to be confused with Jersey, which shared this county's gap record and stays shut: its Jersey 9 and Jersey 10 vanish into an unnamed subset of Jersey 1-8.
    "johnson-county-outline.json": (1, 1),  # Johnson County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. It shipped on 2026-08-04 as gap-location geometry only, from the pass-13 detached-counties sweep, and was promoted on 2026-08-21 when Johnson joined ON ITS PRECINCTS: its board is elected countywide, so there is no district geometry to seek, and its roster stays unshipped because the only county source for it is a website that refuses this client.
    "johnson-precincts.json": (16, 16),  # Johnson County's sixteen voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 16/16 against the county's own precinct names and they sum to its exact 2020 population of 13,308. Bounds are an EQUALITY because sixteen is the county's whole precinct list. THE COUNTY'S OWN WEBSITE WAS NEVER READ — johnsoncountyil.com sits behind a host-level block — and none of this comes from it: the names, the count and the board's form all come from the county's certified 2026 General Primary on results.gbsvote.com, which prints a committeeperson contest per precinct per party and reads "16 OF 16 PRECINCTS REPORTING". NO BOARD DISTRICT ships and none ever will: that canvass carries a single countywide FOR COUNTY COMMISSIONER contest per party over all sixteen precincts and no district-suffixed board contest at all. NO ROSTER ships either — the commissioners are a County-card fact and the only county source for them is the blocked site — and NO POLLING PLACE, by the rule Calhoun's build set (scripts/build_johnson_precincts.py).
    ...
    "lee-library-districts.json": (11, 11),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py. NOT a county or library publication, and it CARRIES A LICENCE rather than nothing: AGOL item 675906ae06b5460296154760b7fb1367 states public use permitted, no warranty, NOT for legal boundary determinations, an ATTRIBUTION requirement (users must cite the source) and a MODIFICATION-DISCLOSURE requirement (users may modify the data but must indicate any changes when sharing derived products). This file IS modified — clipped to the county and simplified at 5 m — so the card discloses both the publisher and those changes. Only LAYER 11's sub-metadata is empty; the service and the item are not. Lee's 11. Five are seated in neighbouring counties and reach across the line; only the Lee slice is drawn, so a reader on either side is answered by their own county's entry.
    ...
    "massac-county-outline.json": (1, 1),  # Massac County outline — the SERVED area's fourth island (2026-08-21). Massac joins through the County card alone: its board is a three-member commission elected countywide, so there is no district geometry and no dispatch entry, and this outline stays dynamic_reference. Shipped as gap-location geometry by the pass-13 sweep (2026-08-04) and promoted here.
    ...
    "menard-county-outline.json": (1, 1),  # Menard County coverage outline — the coverage test for the county's dispatch entries. It shipped as a pass-7 gap-location outline when the county's board geometry was recorded as blocked, was promoted when the county's OWN commissioner-district export arrived and shipped as menard-commissioner-districts.json, and on 2026-08-21 it took on the county-precinct entry beside it. The line this note used to carry — "NOT a dispatched county" — had been false since that export landed.
    "menard-precincts.json": (14, 14),  # Menard County's fourteen voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 14/14 and they sum to its exact 2020 population of 12,297. Bounds are an EQUALITY because fourteen is the county's whole precinct list. UNLIKE THE OTHER SEVEN COUNTIES BUILT THIS WAY, Menard is on no election-results platform this project has found (il-menard.pollresults.net returns that vendor's NotFound shell and GBS does not list the county), so the names come from the Clerk's own publications: a precinct map that is a VECTOR PDF with a readable text layer — rare in this fleet, where such maps are almost always scans — and a polling-place list numbering the same fourteen #1 to #14 across seven buildings. Features are LABELLED from the county's own spelling, which writes NORTH ATHENS CITY with a space where the census hyphenates it and the polling sheet uses a slash. No board district is carried here: Menard's five commissioner districts ship separately from the county's own file. No polling place ships (scripts/build_menard_precincts.py).
    ...
    "il-state-outline.json": (1, 1),  # Illinois itself, from layer 0 of the same TIGERweb MapServer the coverage outline comes from and simplified by the same scripts/build_metro_outline.py at the same tolerance. It draws the MIDDLE band of the out-of-scope wash — inside Illinois but outside the served counties, where the statewide layers (county, township, municipality, school district, ZIP) still answer and the county-level ones do not. Illinois is the only instance in the fleet with a middle band: Wisconsin's coverage IS its state, and San Francisco and New York City have no wider region in play. PRE-BUILT RATHER THAN FETCHED, and that is measured: the rebrand preview queried TIGERweb for this at boot, which is 332 KB over 19,789 vertices — four times the school-board fetch metro-outline.json exists to have removed from the critical chain for this very wash. Same tolerance as the county dissolve on purpose: the two rings trace the same rivers where a served county fronts the state line, and simplified apart they would open slivers of false "unserved Illinois" along the Mississippi and the Ohio.
    ...
    "moultrie-county-outline.json": (1, 1),  # Moultrie County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. It shipped as gap-location geometry, served the County card when Moultrie's nine at-large board members joined on 2026-08-18, and was promoted on 2026-08-21 when its sixteen precincts dispatched — the county's first dispatch entry of any kind.
    "moultrie-precincts.json": (16, 16),  # Moultrie County's sixteen voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 16/16 and they sum to its exact 2020 population of 14,526. Bounds are an EQUALITY because sixteen is the county's whole precinct list, named by its certified 2026 General Primary on il-moultrie.pollresults.net. NO ALIAS is needed though the two sources look different at a glance: the county's feed writes SULLIVAN #1 where the census writes SULLIVAN 1, a numbering glyph that norm() absorbs, and the features are LABELLED from the census basename because '#' is punctuation rather than part of a name. This is Moultrie's FIRST DISPATCH ENTRY OF ANY KIND — it joined the ring on 2026-08-18 through the County card alone. NO BOARD DISTRICT ships and none ever will: the same returns carry the board's contest as COUNTY BOARD DISTRICT AT LARGE MEMBER, the county's own wording, so its nine members ride the County card. No polling place ships (scripts/build_moultrie_precincts.py).
    "municipal-ward-coverage.json": (50, 76),  # Ward-electing municipalities' outlines, tagged by dispatch entry — the cheap same-origin coverage test for every non-Chicago entry of the ward layer, metro or not (build_municipal_ward_coverage.py; Rockford is the first outside the metro, Moline and Silvis the first on the Mississippi, Mendota the first whose seats were already in the roster before its geometry was). 72 municipalities across twenty-six entries as of 2026-09-05, when Madison's six ward cities joined on one county layer; the pass-6 ward tranche (2026-08-02) had taken it to 56 across twenty-one. THE COUNT MOVES WITHOUT A SOURCE BEING ADDED: build_municipal_ward_coverage.py queries TIGERweb's place service UNPINNED, so any rebuild adopts that day's vintage for every municipality already here — Madison's six re-drew 22 unrelated outlines — which is why the builder now names what it redrew and why this bound has room above the current count.
    ...
    "perry-county-outline.json": (1, 1),  # Perry County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. It shipped on 2026-08-03 as gap-location geometry only, from the pass-10 frontier sweep, and was promoted on 2026-08-21 when Perry joined ON ITS PRECINCTS: its board is elected countywide, so there is no district geometry to seek, and its roster stays unshipped because the only county source for it is a website that refuses this client.
    "perry-precincts.json": (27, 27),  # Perry County's twenty-seven voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 27/27 against the county's own precinct names and they sum to its exact 2020 population of 20,945. Bounds are an EQUALITY because twenty-seven is the county's whole precinct list. THE COUNTY'S OWN WEBSITE WAS NEVER READ — it refuses this client — and none of this comes from it: the names, the count and the board's form all come from the county's certified 2026 General Primary on results.gbsvote.com, which prints a committeeperson contest per precinct per party and reads "27 OF 27 PRECINCTS REPORTING". Each feature is LABELLED from the census basename so the county seat keeps its own spelling — the results feed writes DUQUOIN closed up where the census and the city write Du Quoin, a rendering difference that norm() absorbs rather than a different precinct. NO BOARD DISTRICT ships and none ever will: that canvass carries a single countywide FOR COUNTY COMMISSIONER contest per party over all twenty-seven precincts and no district-suffixed board contest at all. NO ROSTER ships either — the commissioners are a County-card fact and the only county source for them is the blocked site — and NO POLLING PLACE, by the rule Calhoun's build set (scripts/build_perry_precincts.py).
    "pulaski-precincts.json": (11, 11),  # Pulaski County's eleven voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 11/11 against the county's own precinct names and they sum to its exact 2020 population of 5,193. Bounds are an EQUALITY because eleven is the county's whole precinct list. THE COUNTY'S OWN WEBSITE WAS NEVER READ — pulaskicountyil.gov resolves to an address this project's egress refuses — and none of this comes from it: the names, the count and the board's form all come from the county's certified 2024 General Primary on platinumelectionresults.com (county id 19), which prints a committeeperson contest per precinct per party. THE COUNTY WAS OPENED BY A SWEEP CORRECTION, not a new source: that vendor had been swept ONE election slug wide and reported as not carrying Pulaski. NO BOARD DISTRICT ships and none ever will: the same canvass carries a single countywide For County Commissioner contest per party over all eleven precincts and no district-suffixed board contest at all, and the 2016 General agrees. NO ROSTER ships either — the commissioners are a County-card fact, the county's site is unreachable, and the vendor holds only a primary (nominees, not officeholders) and one decade-old general — and NO POLLING PLACE, by the rule Calhoun's build set (scripts/build_pulaski_precincts.py).
    "hardin-precincts.json": (6, 6),  # Hardin County's six voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 6/6 against the county's own precinct names and they sum to its exact 2020 population of 3,649. Bounds are an EQUALITY because six is the county's whole precinct list. THE COUNTY PUBLISHES NO WEB PAGE AT ALL — hardincountyil.gov and www.hardincountyil.gov both answer 200 with the same 114-byte script redirect to a parking lander — so none of this comes from the county's site: the names, the count and the board's at-large form all come from its certified 2026 General Primary on il-hardin.pollresults.net, which prints a committeeperson contest per precinct per party (twelve contests naming all six, each 1 of 1) while every countywide office reports 6 of 6. A SECOND VENDOR CORROBORATES THE FORM a decade apart: platinumelectionresults.com county id 20 carries a 2016 General with For County Commissioner over 6 of 6. THIS COUNTY ALSO CORRECTED A BAR THIS PROJECT INVENTED FOR AN HOUR — Pulaski's build log said Hardin lacked a 'second naming source', which is a standard Johnson, Perry and Pulaski were never held to. NO BOARD DISTRICT ships and none ever will. NO ROSTER ships either — a primary names nominees, not officeholders, and the county publishes no page — and NO POLLING PLACE, by the rule Calhoun's build set. TWO PRECINCTS CARRY AN EXPLICIT LABEL rather than a derived one, each with a source: McFarlan from the census's own mixed-case county-subdivision layer (title_case would render 'Mcfarlan'), and Cave In Rock from the county's returns, where the census hyphenates it and the county does not (scripts/build_hardin_precincts.py).
    ...
    "pulaski-county-outline.json": (1, 1),  # Pulaski County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. It shipped on 2026-08-04 as gap-location geometry only, from the pass-13 detached-counties sweep, and was promoted on 2026-08-23 when Pulaski joined ON ITS PRECINCTS: its board is elected countywide, so there is no district geometry to seek, and its roster stays unshipped because the only county source for it resolves to an address this project's network refuses.
    ...
    "randolph-library-districts.json": (9, 9),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py. NOT a county or library publication, and it CARRIES A LICENCE rather than nothing: AGOL item 675906ae06b5460296154760b7fb1367 states public use permitted, no warranty, NOT for legal boundary determinations, an ATTRIBUTION requirement (users must cite the source) and a MODIFICATION-DISCLOSURE requirement (users may modify the data but must indicate any changes when sharing derived products). This file IS modified — clipped to the county and simplified at 5 m — so the card discloses both the publisher and those changes. Only LAYER 11's sub-metadata is empty; the service and the item are not. Randolph's 9, a mix of two large districts and seven municipal libraries — the county is 27.6% covered and the rest genuinely sits in no library district.
    "richland-county-outline.json": (1, 1),  # Richland County coverage outline — the coverage test for the county's board and precinct dispatch entries (richlandCountyCoverage). Shipped 2026-08-04 as gap-location geometry by the pass-13 detached-counties sweep, when Richland read as source-less; promoted to a coverage outline on 2026-08-23 when the county joined as the 86th served county, so index.html now names it literally and it is no longer a dynamic reference.
    ...
    "saline-county-outline.json": (1, 1),  # Saline County outline — the 74th served county (2026-08-21). Saline joins through the County card alone: thirteen members elected countywide, so there is no district geometry and no dispatch entry, and this outline stays dynamic_reference. A mainland join, bordering the already-served Hamilton and White.
    ...
    "sangamon-library-districts.json": (16, 16),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py. NOT a county or library publication, and it CARRIES A LICENCE rather than nothing: AGOL item 675906ae06b5460296154760b7fb1367 states public use permitted, no warranty, NOT for legal boundary determinations, an ATTRIBUTION requirement (users must cite the source) and a MODIFICATION-DISCLOSURE requirement (users may modify the data but must indicate any changes when sharing derived products). This file IS modified — clipped to the county and simplified at 5 m — so the card discloses both the publisher and those changes. Only LAYER 11's sub-metadata is empty; the service and the item are not. Sangamon's 16, including Springfield's Lincoln Library as a CITY library rather than a district. Farmersville-Waggoner's 3,444 m2 border sliver is dropped: it never reaches 10 m inside the county and its body is seated in Montgomery.
    ...
    "schuyler-county-outline.json": (1, 1),  # Schuyler County coverage outline — the coverage test for the county's county-precinct dispatch entry, and NEW on 2026-08-21. Schuyler reached the ring on 2026-08-02 through the County card alone, which needs no coverage test, so the county had no outline at all until its seventeen precincts dispatched. Anchors derived from TIGERweb Incorporated Places centroids inside the county and inside each of its six neighbours, each point-in-county tested (scripts/build_county_outline.py).
    "schuyler-precincts.json": (17, 17),  # Schuyler County's seventeen voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 17/17 after one alias and they sum to its exact 2020 population of 6,902. Bounds are an EQUALITY because seventeen is the county's whole precinct list. THE CLERK-DOMAIN RULE DOES NOT REACH THIS COUNTY: schuylercountyil.gov, the address in il-county-clerks.json, has NO A RECORD AT ALL, and the county's real site is www.schuylercounty.org — which this repo has been scraping for the board roster since 2026-08-02, so the answer was already here. The name list is the county's "Polling Places by Precinct" table, whose three "&" rows each serve two precincts from one building; the COUNT is confirmed by the county's OWN results site (elections.schuyler.il.us — the second county-run results database this project has found, after Hancock's), which reads "Precincts Reporting 17 of 17" on an official 2026 General Primary and names three of the seventeen through committeeperson contests. ONE ALIAS and the CENSUS is at fault: it writes FREDRICK where the county and the village write Frederick. Buena Vista keeps its space, which census and results feed both drop. NO BOARD DISTRICT ships and none ever will — the same canvass carries a single countywide MEMBERS OF THE COUNTY BOARD contest over all seventeen precincts, so Schuyler's seven members ride the County card. No polling place ships (scripts/build_schuyler_precincts.py).
    "scott-county-outline.json": (1, 1),  # Scott County coverage outline — the coverage test for the county's county-precinct dispatch entry, and what puts the county inside the coverage ring. It shipped as gap-location geometry beside the 7th-Circuit subcircuit that first served this county, and was promoted on 2026-08-21 when its ten precincts dispatched.
    "scott-precincts.json": (10, 10),  # Scott County's ten voting precincts — the Census 2020 voting districts ONE FOR ONE rather than a dissolve: the Jasper test passes 10/10 and they sum to its exact 2020 population of 4,949. Bounds are an EQUALITY because ten is the county's whole precinct list, named by its certified 2026 General Primary one committeeperson contest at a time under a header reading "10 OF 10 PRECINCTS REPORTING". ONE SPELLING IS RECORDED RATHER THAN CHOSEN, and it is decided the OPPOSITE way from Greene's: the county's results feed writes MERRIT where the census writes MERRITT, and MERRITT is the township's name, so the feed dropped a letter rather than the census adding one. The alias exists so the Jasper test can compare the county's own list while the shipped LABEL comes from the census name the alias displaces — apply_aliases records that as censusName. The Winchester roman numerals, by contrast, are REAL: county and census both write WINCHESTER I, II and III, unlike Cumberland's, which turned out to be polling-place groupings. NO BOARD DISTRICT ships and none ever will — a single countywide COUNTY COMMISSIONER contest over all ten precincts. No polling place ships (scripts/build_scott_precincts.py).
    ...
    "shelby-precincts.json": (33, 33),  # Shelby County's 33 voting precincts — the Census 2020 voting districts, which carry the county's ADOPTED County Board map's own 33 precinct names 33/33 and sum to its exact 2020 population of 20,990. Bounds are an EQUALITY because the fabric is the county's whole precinct list. That 33/33 agreement is exactly what makes the district dissolve legitimate, so these precincts were buildable from the day the districts shipped; the absence simply had no gap record explaining it (the Cass/Greene/Scott/Moultrie shape). Thirty-two features carry a single County Board district. SHELBYVILLE 5 CARRIES THREE: it wraps around the city of Shelbyville and the adopted plan divides it between Districts 8, 9 and 10 along two landmarks the county names (Route 16 and Lake Shelbyville). The PRECINCT is not divided — a voter in Shelbyville 5 is in Shelbyville 5 — so it ships whole and names the three districts it spans rather than picking the largest of its three pieces. No polling place ships (the Calhoun rule; scripts/build_shelby_board_districts.py).
    ...
    "st-clair-library-districts.json": (19, 19),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py. NOT a county or library publication, and it CARRIES A LICENCE rather than nothing: AGOL item 675906ae06b5460296154760b7fb1367 states public use permitted, no warranty, NOT for legal boundary determinations, an ATTRIBUTION requirement (users must cite the source) and a MODIFICATION-DISCLOSURE requirement (users may modify the data but must indicate any changes when sharing derived products). This file IS modified — clipped to the county and simplified at 5 m — so the card discloses both the publisher and those changes. Only LAYER 11's sub-metadata is empty; the service and the item are not. St. Clair's 19, the densest of the six. East Saint Louis, Belleville and O'Fallon are municipal libraries; New Athens, Marissa and Freeburg are districts.
    ...
    "stephenson-library-districts.json": (5, 5),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py. NOT a county or library publication, and it CARRIES A LICENCE rather than nothing: AGOL item 675906ae06b5460296154760b7fb1367 states public use permitted, no warranty, NOT for legal boundary determinations, an ATTRIBUTION requirement (users must cite the source) and a MODIFICATION-DISCLOSURE requirement (users may modify the data but must indicate any changes when sharing derived products). This file IS modified — clipped to the county and simplified at 5 m — so the card discloses both the publisher and those changes. Only LAYER 11's sub-metadata is empty; the service and the item are not. Stephenson's 5. Pearl City Public Library District appears here AND in Carroll's file, each with its own county's slice — the district straddles the line and neither county's reader loses their answer.
    ...
    "vermilion-county-outline.json": (1, 1),  # Vermilion County coverage outline — the coverage test for the county's board dispatch entry (vermilionCountyCoverage). Shipped 2026-08-03 as gap-location geometry by the pass-10 frontier sweep; promoted to a coverage outline on 2026-08-23 when the county joined as the 89th served county, so index.html now names it literally and it is no longer a dynamic reference.
    ...
    "wayne-county-outline.json": (1, 1),  # Wayne County coverage outline — the coverage test for the county's board and precinct dispatch entries (wayneCountyCoverage). Shipped 2026-08-04 as gap-location geometry by the pass-13 detached-counties sweep, when Wayne read as source-less; promoted to a coverage outline on 2026-08-25 when the county joined as the 90th served county, so index.html now names it literally and it is no longer a dynamic reference.
    ...
    "hancock-county-board-districts.json": (5, 5),  # Hancock County's 5 County Board districts (seating 15, three each), DISSOLVED from the Census 2020 voting districts per a composition the county's OWN CERTIFIED RETURNS state precinct by precinct. The source is the difference: Hancock's Clerk runs a results database at electionstats.hancockcounty-il.gov — the county's own, not a vendor's — and every contest page carries a PER-PRECINCT results table, so a district contest NAMES the precincts that voted in it one row at a time instead of only counting them. That is exactly what Knox's and Richland's canvasses withhold, and it is what makes this county buildable and those two not. TWO WITNESSES PER DISTRICT, the Clark standard: the 2022 and 2026 primaries carry all five district contests and agree exactly, with the 2024 primary re-confirming four of the five. THE PARTITION IS EXACT — 6+8+8+5+6 = 33 precinct slots, 33 distinct names, none shared and none left over against a county that runs 33 precincts. THE JASPER TEST PASSES 33/33 with the population identity to the person: the census voting districts sum to Hancock's exact 17,620. Thirteen names differ in SPELLING ONLY — the roman/arabic convention already recorded for Clay and Cumberland (CARTHAGE I for Carthage 1) — and ONE OF THE THIRTEEN IS A CENSUS TYPO, MONTIBELLO IV for Montebello 4, aliased like the rest and named so nobody reads it as a different place. Populations run 3,345-3,673 against a 3,524 ideal, 5.1% worst and 9.3% max-to-min (scripts/build_hancock_boundaries.py; --check is the drift gate).
    "hancock-precincts.json": (33, 33),  # Hancock County's 33 voting precincts — the Census 2020 voting districts, which ARE the county's own fabric by measurement rather than assumption: they carry its 33 precinct names 33/33 as its certified results database prints them, and their POP100 sums to its exact 2020 population of 17,620. Bounds are an EQUALITY because the fabric is the county's whole precinct list. NOTHING NEW WAS SOURCED FOR THIS FILE: the composition that already draws the county's five board districts states which district each precinct belongs to, so the precincts were buildable from the day the districts shipped. The absence simply had no gap record explaining it — the Cass/Greene/Scott/Moultrie shape, where a served county goes unbuilt for weeks while every guard stays green, and the reason the audit that found it is worth repeating. THE COUNTY'S SPELLING SHIPS: thirteen census names differ by the roman/arabic convention (CARTHAGE I for Carthage 1) and one is a census TYPO, MONTIBELLO IV for Montebello 4; where the two differ the census's own name rides the feature as censusName, so a reader can still find the precinct in census products and the typo is visible rather than silently corrected away. Every feature carries its County Board district, because Hancock's districts are unions of WHOLE precincts. No polling place ships (the Calhoun rule; scripts/build_hancock_boundaries.py).
    "richland-county-board-districts.json": (7, 7),  # Richland County's 7 single-member County Board districts, DISSOLVED from the county's own precinct fabric (Census 2020 VTDs) per a composition read off THE COUNTY'S OWN GIS — the first county in the fleet composed from geometry rather than from election returns. Richland's canvasses COUNT precincts per district and never name one (the Knox shape), which is why this county sat as a gap for weeks; richlandil.wthgis.com, linked from the county's own site, publishes 17159countyboarddistricts.tml and votingprecincts.tml as separate county-authored layers, and overlaying them puts every one of the 21 precincts inside exactly one district at 98.4-100% of its area, with no precinct holding more than 0.89% in any second district. THREE INDEPENDENT WITNESSES AGREE: the certified 2024 General counts 4, 4 and 2 precincts in districts 1, 3 and 5 and this composition gives exactly those; the seven populations sum to the county's exact 15,813 and run 2,066-2,494 against a 2,259 ideal (worst deviation 10.4%); and the county's published precinct map, illegible where Olney's lines cut but legible in the countryside, agrees everywhere it can be read. THIS DISPROVES THE COUNTY'S OWN GAP RECORD, which had read nine of its 21 precincts reporting as two ISBE sub-units each as evidence that district lines run THROUGH precincts — what those sub-units actually are is still unknown and is not guessed at. The weekly roster run re-reads both GIS layers and fails if either stops matching this dissolve; full re-verification is scripts/build_richland_boundaries.py --check (operator step: requests/shapely).
    "richland-precincts.json": (21, 21),  # Richland County's 21 voting precincts — the Census 2020 voting districts, which ARE the county's own fabric and are measured to be TWICE OVER: the names match the 21 its certified 17 March 2026 General Primary ballot publication prints as committeeperson contests 21/21 with no alias, their POP100 sums to its exact 2020 population (15,813), and each precinct polygon in the county's own GIS is 98.8-100.0% contained in the same-named voting district. Bounds are an EQUALITY because the fabric is the county's whole precinct list. Each feature carries its census GEOID, POP100 and its County Board district, which is whole-precinct — no precinct is split. The county's GIS and polling list write the county seat's precincts long ('Olney Precinct 5') where its ballot and the census write 'OLNEY 5'; the shipped label follows the ballot, because that is the form the county itself certifies. NO POLLING PLACE ships: the county publishes a good list, and a polling place is a roster fact rather than geometry (the Calhoun rule; scripts/build_richland_boundaries.py).
    "jackson-county-board-districts.json": (7, 7),  # Jackson County's 7 County Board districts, each electing TWO members (14 seats), and the FIRST in the fleet whose districts SPLIT precincts and are built anyway. Fifty-three of the county's 56 precincts take their district from its certified 5 November 2024 General canvass, which lists for each district's contest exactly the precincts that voted in it. THREE ARE REPORTED TWICE, under two districts each with their registration divided — Carbondale 21 (D6 600 / D7 282), Carbondale 24 (D5 609 / D6 98) and Murphysboro 4 (D3 166 / D5 391) — which is the county stating that a district line runs through them, and is why this county was recorded measured-shut beside Bureau, Cumberland, Douglas, Ford, Knox and Piatt. What that record missed is that it named the answer itself: the county publishes its adopted Board District Map as a VECTOR PDF whose seven districts are seven FILLED PATHS in the content stream — polygon objects with exact fill colours pairing one-for-one with the legend in legend order, not pixels to be classified (the method section warns against colour-sampling a RASTERISED map, and this is the opposite of that). The map is georeferenced to EPSG:3436 (Illinois State Plane West), established by fitting the union of the seven districts against TIGER's county boundary across candidate CRSs — 3436 matched the aspect ratio to four decimals and no other candidate did — and refined against the 53 unambiguous precincts; mean boundary offset 5.3 m, with the map's own scale bar independently confirming the scale. NOTHING TRACED SHIPS: the boundary is census geometry throughout, voting districts for the 53 whole precincts and whole census BLOCKS inside the three split ones, and the map is only ever allowed to choose between the two districts the canvass already names. FOUR INDEPENDENT VERIFICATIONS: all 53 unambiguous precincts land in the district the canvass names (53/53, at 98.3-100% of area); all three split precincts split between exactly the two it names (3/3); populations run 7,371-7,933 against a 7,568 ideal, worst deviation 4.83%, which a mis-assigned split precinct would destroy; and Murphysboro 4's census population divides 30.2/69.8 where its registration divides 29.8/70.2. THAT LAST CHECK IS DELIBERATELY NOT CLAIMED FOR THE OTHER TWO — Carbondale 21 and 24 hold SIU dormitories and student apartments, one block carrying 700 people in FOUR housing units, so census population does not predict registration there in either direction. Inside the three split precincts only FIVE blocks holding SIXTEEN people are less than 95% resolved by the georeferencing (scripts/build_jackson_boundaries.py; --check is the drift gate).
    "jackson-precincts.json": (56, 56),  # Jackson County's 56 voting precincts — the Census 2020 voting districts, which ARE the county's own fabric and are measured to be: the names match its certified 2024 General canvass 56/56 and their POP100 sums to its exact 2020 population (52,974). Bounds are an EQUALITY because the fabric is the county's whole precinct list. Each feature carries its census GEOID and POP100. Most carry their County Board district; THREE carry a LIST of two instead — Carbondale 21, Carbondale 24 and Murphysboro 4 are split between districts, which the county's own certified canvass states by reporting each of them twice with its registration divided. For those three the precinct genuinely does not determine the district and the card says which two it could be rather than picking one. Every one of the county's 2,773 census blocks nests 99%+ inside exactly one of these precincts, which is what makes the block-level resolution of the three splits well defined. NO POLLING PLACE ships: a polling place is a roster fact rather than geometry (the Calhoun rule; scripts/build_jackson_boundaries.py).
    "douglas-county-board-districts.json": (7, 7),  # Douglas County's 7 single-member County Board districts, and the FIRST in the fleet shipped exactly as the county draws them — no dissolve, no georeferencing, nothing derived. The county's GIS publishes CountyBoardDistricts as a PUBLIC ArcGIS feature service (services3.arcgis.com/HXjsHxkFFcZquC8P), shared with empty licenseInfo and no copyright text, and this ships those polygons. NOBODY HAD LOOKED: the county's ArcGIS web map names ONE parcel service in its config while the ORG BEHIND IT carries FIFTY-FOUR — a viewer shows what it uses, the org shows what the county has, which is the Richland finding for a second time. That matters here because Douglas SPLITS PRECINCTS: two of its seventeen vote in two districts each, so no dissolve of whole precincts could ever have drawn it, and the county's own polygons are the only thing that can. VERIFIED AGAINST A DIFFERENT COUNTY PRODUCT rather than described: its certified 8 November 2022 General Election Summary Report ran all seven districts and printed the NUMBER of precincts voting in each — 2, 2, 3, 2, 3, 2, 5, nineteen across a county of seventeen because the two split ones count twice — and overlaying the county's precinct layer on its district layer reproduces all seven exactly, including both splits (Bourbon 2 at 60.0%/39.9% between districts 1 and 3, Bowdre at 62.5%/37.5% between 5 and 7). That check is a GATE in the builder, not a comment. Three more: the layer's own TOTAL_POP sums to 19,740, the county's exact Census 2020 count, running 2,504-3,230 against a 2,820 ideal; the seven tile 99.97% of the county with 2,853 m2 of pairwise overlap and 92,043 m2 spilling outside the county line, both digitisation noise between independently drawn county layers and both held under ceilings; and the census voting districts carry the county's own precinct names 17/17. Each district names the precincts wholly inside it separately from those only partly in it (scripts/build_douglas_boundaries.py; --check is the drift gate).
    "vermilion-county-board-districts.json": (9, 9),  # Vermilion County's 9 County Board districts, three members each (27 seats), shipped exactly as the county draws them. THE COUNTY WAS NEVER BLOCKED: its gap record said the site "is not reachable from this project's network", which was true of a TLS handshake and false of the county — vercountyil.gov redirects to www.vercounty.org, which serves the COLES PATTERN, its leaf certificate without the GoGetSSL intermediate that signed it. The boundaries did not even need the site: the county's GIS publishes to an ArcGIS Online org (services6.arcgis.com/am689ZyfXfdo9vCK) carrying twenty-six public services, three of which draw board districts. THE WELL-LABELLED LAYER IS THE OBSOLETE ONE and that is this county's trap — a service described as "Governmental and school districts in Vermilion County IL" holds the superseded map, identical to a CountyBoardDistricts layer whose Name/Party/Elected/Email columns stop at 2018, while the CURRENT map is an undocumented layer with a typo in its own name, CountyBoardDistrcts2021. WHICH IS CURRENT IS A MEASUREMENT, NOT A READING OF ITS NAME, and it is a GATE in both directions: assigning all 4,943 Census 2020 blocks by centroid, the shipped layer runs 8,209-8,298 against an 8,243 ideal (worst 0.7%) and the better-labelled one 7,609-9,484 (worst 15.1%), and the builder refuses to write if the superseded layer ever comes into balance too. THREE INDEPENDENT WITNESSES: both election authorities' certified returns name all nine districts, the nine tile 99.98% of the county, and every one of its 74,188 residents lands in exactly one district. THE TWO AUTHORITIES NAME CONTESTS DIFFERENTLY and reading one convention hides a district — the County Clerk spells them out ("COUNTY BOARD 8TH DISTRICT MEMBER") and the Danville Election Commission abbreviates ("CO. BD. MEMBER D8"), so four of the clerk's canvasses show districts 1-8 and report DISTRICT 9 AS NOT EXISTING; it is in the commission's 17 March 2026 General Primary (scripts/build_vermilion_boundaries.py; --check is the drift gate).
    "douglas-precincts.json": (17, 17),  # Douglas County's 17 voting precincts — the Census 2020 voting districts, which ARE the county's own fabric and are measured to be against the county's OWN VotingDistricts feature service rather than a transcribed list: 17/17 names, and POP100 summing to its exact 2020 population of 19,740. Bounds are an EQUALITY because the fabric is the county's whole precinct list. Most features carry their County Board district; TWO carry a LIST of two instead — Bourbon 2 and Bowdre are split between districts, which the county states twice over: its own district layer cuts through them, and its certified 2022 General counts nineteen precinct-slots across seven districts in a county of seventeen precincts. For those two the precinct genuinely does not determine the district and the card says which two it could be rather than picking the larger share. NO POLLING PLACE ships (the Calhoun rule; scripts/build_douglas_boundaries.py).
    "wayne-county-board-districts.json": (7, 7),  # Wayne County's 7 County Board districts, each electing TWO members (14 seats), dissolved from Census 2020 voting districts per a composition TWO SOURCES AGREE ON: the county's own board page states outright which precincts make up each district, and the county's certified 5 November 2024 General — 27 precinct-level results grouped by which board candidate each precinct voted on — partitions the same 27 precincts into the same seven groups. THE ONE DIFFERENCE IS THE COUNTY SEAT: the board page's District 7 lists only Merriam and Golden Gate, five precincts short of the ballots' 27, and the two it omits — Fairfield 1 and Fairfield 2 — voted in District 7's contest on that same certified ballot, so the page is incomplete rather than the ballots wrong. WHY THIS SAT AS A GAP FOR WEEKS AFTER THE COMPOSITION WAS SETTLED: population, not provenance. Against a 2,311 ideal the seven districts run 1,863 (D6) to 3,060 (D3) — D3 alone deviates 32.4%, past this project's usual 30% mis-assignment ceiling, and both sources agree on D3's composition exactly. Clerk Elizabeth Woodrow was asked directly whether the plan is current or has moved since it was drawn; her reply, 2026-08-25: "Nothing has changed with this map for years." The ceiling is raised for this county alone, with the measured 32.4% recorded rather than the default silently widened (scripts/build_wayne_boundaries.py; --check is the drift gate).
    "wayne-precincts.json": (27, 27),  # Wayne County's 27 voting precincts — the Census 2020 voting districts, which ARE the county's own fabric and are measured to be: the names match the county's own 27/27 (one spelling aliased, Massillon/Massilon) and their POP100 sums to its exact 2020 population of 16,179. Bounds are an EQUALITY because the fabric is the county's whole precinct list. Each feature carries its census GEOID, POP100 and its County Board district, which is whole-precinct — no precinct is split between districts. NO POLLING PLACE ships: nothing the county publishes pairs its precincts with buildings as data (the Calhoun rule; scripts/build_wayne_boundaries.py).
}
...
ROSTER_FILES = {
    ...
    "adams-county-board-members.json": 8,  # Adams County Board members keyed by district (7 districts of THREE seats each, 21 in all) plus a top-level 'board' entry. PARTIAL BY CONSTRUCTION AND SAYS SO ON THE CARD: the county's own board page is a flat Akamai deny and the Internet Archive never captured it, so the roster is read from CERTIFIED ELECTION CANVASSES on platinumelectionresults.com — the THIRD results vendor, which carries Adams as county id 54 and which this county's gap record had never tested (it tested the pollresults/accessliberty pair, correctly found them empty, and stopped). Adams staggers 2 seats per district one cycle and 1 the next, so the November 2024 general names ONE of each district's three members and the other two were seated in November 2022 — a canvass the vendor returns as an empty placeholder for 2021, 2022 and 2023, which Clarity does not carry and which the county publishes only behind the same WAF. Every row renders the election that seated it (districtSource) because a canvass cannot show a mid-term appointment, and the card states the shortfall in words rather than leaving a short list to imply it. No chair is badged (ISBE's chair column measured wrong in 16 of 56 counties — the Coles rule) and no per-member contact ships, because it exists only on the unreadable page. The weekly run re-fetches the 2022 slug every time and reports it unread, so the day the county backfills it fourteen members appear instead of the gap quietly persisting (scripts/adams_county_board_scraper.py + build_adams_county_board_roster.py).
    "knox-county-board-members.json": 6,  # Knox County Board members keyed by district (5 districts of THREE seats each, 15 in all) plus a top-level 'board' entry. THE ROSTER IS THE ROLL CALL OF THE COUNTY'S OWN BOARD MINUTES: knoxcountyil.gov answers 403 to every automated client and the Archive never captured its member page, but the same website's DOCUMENTS are served plainly from its Revize CMS, and every set of minutes opens by naming each seat with its district. Absent members are parsed exactly like present ones — a seat is not vacated by missing a meeting, and reading only the present block would have shipped fourteen of fifteen and called the county complete. Every row names the meeting it came from, because a roll call is one evening's record and cannot show a member seated the week after. The Chair is badged from the same minutes, which state it outright. No party, term, phone or e-mail ships: the minutes publish none and the page that does cannot be read, so the county's Clerk line stands for the board (the Calhoun/Clark switchboard posture). Attendance never ships (scripts/knox_county_board_scraper.py + build_knox_county_board_roster.py).
    ...
    "logan-county-board-members.json": 7,  # Logan County Board members keyed by district (6 two-member districts, 12/12 with phone and e-mail; the Chair and Vice Chair tags ride their member rows — the county says who holds them) — scraped weekly from the county's own board page. Its existence retired the entry's rule-4 branch-3 honesty floor. PLUS A TOP-LEVEL 'board' ENTRY (2026-09-04) carrying the county's own "Logan County Board Office" block — street address, direct line and board mailbox — which is why min_keys is 6+1 rather than 6: the block is a merge gate, not merely present. The same page prints a DIFFERENT home address beside each of the twelve members and none of those is parsed or shown (the Madison/Peoria rule), so the one address that ships is the one the county itself labels as the board's.
    ...
    "grundy-county-board-members.json": 4,  # Grundy County Board members keyed by district (three districts, six seats each — each district ships its `seats`, so a seat the county stops listing renders as 'N of 6 seats not listed in the county's directory' rather than silently shortening the card; the builder floors the board at 16 of 18 named, because three missing is a parser that stopped finding rows rather than a departure. The note says 'not listed', never 'vacant' — the page carries no vacancy marker, it simply stops listing a person, so we can see that a seat is unaccounted for and cannot see why). Named seats carry party, the page's 'Board Member Since' year, committee assignments verbatim, phone and e-mail; the Board Chairman tag rides the chair's own row as the page states it — scraped weekly from the county's own board page. A fourth key, `board`, carries the office the county's own County Board page publishes under an 'Address:' label with the board's phone and fax. AN EARLIER SWEEP REJECTED THAT ADDRESS as site chrome, because grundycountyil.gov's home page carries the same street — under 'Administration Office', which is another department's block; the county's contact page lists SIX distinct department addresses, so these blocks are per-department and the home-page test can only ever settle an address found by proximity, never one the county attributes in words.
    ...
    "il-county-commissioners.json": 19,  # At-large county boards, keyed like il-county-clerks.json. Counties that elect their board COUNTYWIDE have no district geometry, so their members ride the COUNTY card rather than a county-board dispatch entry (EXPANSION_GUIDE §1.5). Nineteen counties, 100 members, from THREE kinds of source. Scraped from a county page: Monroe, Randolph and MORGAN (commission form, 3 each), MASSAC 3, Pike 9, MOULTRIE 9, Brown 7, Schuyler 7, GREENE 7, SALINE 13, Calhoun 5, Putnam 5, Hamilton 5, GALLATIN 5 and ALEXANDER 2. Carried from a document because the county has no readable website: EDWARDS 3 and WABASH 3. Read from certified returns because the county site is behind a captcha: UNION 5 and WILLIAMSON 3 (RETURNS_ROSTERS; each row names the election that seated it, since a canvass cannot show a mid-term appointment). ALEXANDER, added 2026-08-23, is the one county here whose board is BIGGER than its roster — three seats, two published, nobody naming the third — so its block carries `seats` beside its members and the County card prints the shortfall in the same words the districted counties use. It is also the reason MIN_MEMBERS stopped applying to counties that have an EXPECT_MEMBERS row: an exact count is a better guard than a range, and the range would have rejected a correct two-member read. Read the count from the builder's EXPECT_MEMBERS table, never from this sentence. Greene, added 2026-08-08, is the first of these that was ALREADY SERVED before its board arrived (a 7th-Circuit subcircuit answered there), so it moved tier without changing the coverage ring or any anchor. Edwards has no website at all (its Clerk said so on 2026-08-06, and the domain answers NOERROR with no A record), so its three commissioners come from a document she sent. Wabash, added 2026-08-17, is the second such county — wabashcounty.illinois.gov carries mail and serves no web page (port 80 answers 503, measured 5 Aug and re-checked 9 Aug) — and its three come from Clerk Will's e-mail of 2026-08-16; her e-mail lists only HOME addresses, which do not ship, so each Wabash row is a name alone, and nobody is marked Chairman until she answers which of the three chairs. The scraper carries both counties in DOCUMENT_ROSTERS and prints a NOT RE-READ line naming each document and its age on every run, because a weekly job that refreshes a hand-carried roster refreshes nothing. Each shipped county carries EITHER a sourceUrl or a sourceDocument+verified pair; the builder fails on a county with neither. MORGAN, added 2026-08-08, is the cautionary one: the county has TWO sites and this project read the wrong one for a day — morgancounty-il.GOV is a content-free React shell, while morgancounty-il.COM publishes all three with role, party, term dates and a personal e-mail apiece. GALLATIN, added 2026-08-21, is the only one of the nineteen that also carries a dispatch entry of its own for precincts alone, and the only one whose site had to be repaired to be read at all: gallatinco.illinois.gov omits its TLS intermediate, so the scraper chases the issuer the certificate itself names and pins its hash (AIA_INTERMEDIATES) rather than disabling verification. Its board page publishes a phone for two of the five and an e-mail for none, and nothing is invented for the other three.
    ...
    "jo-daviess-county-board-members.json": 18,  # Jo Daviess County Board members keyed by district (17 single-member districts — the seat count the boundary module exports, so the roster and the purchased geometry cannot drift apart silently). Scraped weekly from the county's own board page at jodaviesscountyil.gov (NEVER the old jodaviess.org, which answers every URL with its home page): name, party and term on every seat, Chair/Vice-Chair badged, plus a direct phone and an e-mail per member from each member's own directory page — 16 distinct numbers at first ship, so no switchboard hoist, and personal gmail/icloud addresses carried exactly as published. A seat the county prints as VACANT ships as a counted, never named vacancy (District 16 at first ship, the Livingston posture). The directory pages also print members' HOME ADDRESSES, which the scraper never parses (the Madison/Peoria precedent). The count guard counts SEATS (named + vacant = exactly 17, districts 1..17 each once), so a vacancy is distinguishable from a parse regression — and a changed district count reads as a reapportionment, which would need the purchased boundary re-licensed. PLUS A TOP-LEVEL 'board' ENTRY (2026-09-04) carrying WHERE THE BOARD MEETS rather than an office: the county's words are "County Board room, Third Floor, Jo Daviess County Courthouse", and the card's label reads "Board meets at" for that reason. A meeting room is not an office, and this is precisely the county whose per-member pages print the HOME addresses noted above — so the distinction is drawn in the label rather than blurred. min_keys is 17+1.
    ...
    "hancock-county-board-members.json": 5,  # Hancock County Board members keyed by district (5 districts seating 15, three each, with a party letter per member). Scraped weekly from the county's board page, which publishes a name and a party and NOTHING ELSE — no e-mail, no phone, no term, and no chairman is named, so no row is badged one and there is no contact floor to set. THE COUNT IS THE GUARD for exactly that reason: with no field to watch, a page reshape shows up as a wrong number of members. Two parser traps are handled by construction. The page's <meta name="description"> REPEATS District One's three members verbatim, so anchoring on the first 'District One' in the document parses the meta tag and ships three members as a whole board; the body is found by scanning for the last occurrence. And the last district has no following <h2> to bound it — an unbounded region runs to the end of the document and swallows the site's footer navigation, which parsed District Five as 35 'members', most of them menu links. Each district is bounded to its own first <ul> instead, and a per-district ceiling fails the run loudly if that ever stops holding.
    "richland-county-board-members.json": 7,  # Richland County Board members keyed by district (7 single-member districts = 7 seats). Scraped weekly from the county's own board page, which publishes each member as a card carrying a name, a district line and a county e-mail — 7/7 carry one — and badges the Chairman. No party, term or phone ships because the page publishes none. THE PARSER IS SCOPED TO THE ROSTER BLOCK ON PURPOSE: the same page carries a committees section above it whose 'Chair:' and 'Members:' lines name people who do not sit on the board at all — five community members on the 708 Mental Health Committee alone — so a greedy read would ship a fourteen-name board for a seven-seat county. THE SAME WEEKLY RUN IS THE REDISTRICTING TRIPWIRE: it re-reads the county's own GIS board-district and voting-precinct layers and fails if either stops matching the shipped dissolve, which is this county's only automatic warning that its lines moved. What that check can and cannot see is stated in the builder: it verifies the county still publishes SEVEN districts over the same TWENTY-ONE precinct names, and does not re-run the spatial overlay, which needs the census fabric and does not belong in a weekly roster job.
    "jackson-county-board-members.json": 8,  # Jackson County Board members keyed by district (7 districts electing TWO members each = 14 seats). Scraped weekly from the county's own board page, which groups members under 'District N Board Members' headings, plus each member's staff-directory entry for the county e-mail address — 14/14 carry one. THE HEADING IS THE AUTHORITY FOR THE DISTRICT, and that is this county's trap: a member's job title is their DISTRICT for twelve of the fourteen and their OFFICE for the other two, so the Chair's and Vice Chair's cards name no district at all and a parser keying on the title drops the two most prominent members and reports twelve seats for a fourteen-seat board. The directory entry states the district a second time, and the builder FAILS the run when the two county surfaces disagree rather than picking a winner. No party, phone or address ships because the county publishes none per member, and neither does the term length: the directory prints '2 Year Term' and '4 Year Term' but never a start or end date, and a term that cannot be placed in time tells a reader nothing. An eighth key, `board`, carries what the county publishes at the BOARD level rather than per member: a contact block headed 'County Board' with a Physical Address, the board's phone and its office hours, plus the second-floor Courtroom 4 where the page says the board MEETS — shipped as two separate rows, because an office and a meeting room are different places and one address must not stand for both.
    "douglas-county-board-members.json": 7,  # Douglas County Board members keyed by district (7 single-member districts = 7 seats). THE COUNTY PUBLISHES NO BOARD MEMBERS PAGE AT ALL — no member list, no district pages, nothing in its navigation — so the roster is assembled weekly from the two county documents that each answer half of it, found through the WordPress backend behind its Next.js site rather than hardcoded: the annual COUNTY YEARBOOK names the seven people and gives each a county e-mail, and the certified ELECTION SUMMARY REPORTS say which numbered district each of them won, in the county's own numbering. THE YEARBOOK'S OWN NUMBERS ARE NOT DISTRICT NUMBERS and that is this county's trap: it lists members '1. Tom Hettinger', '2. Bibby Appleby', which reads exactly like a district column, while the certified returns put Hettinger in District 3 and Appleby in District 2 — taking list position as district would misplace SIX of the seven. Its per-member precinct lists cannot recover the district either; they contain at least one error, naming 'Camargo 1 & 2' for a member whose district holds Camargo 2 and 3. So the join is on SURNAME and the builder FAILS when the two documents stop naming the same seven people. Each row carries the election that seated the member, because a certified return names who WON rather than who holds the seat today (the Scott reasoning) and the yearbook is what corroborates that they still do. SIX OF SEVEN E-MAILS SHIP: the yearbook prints District 1's address on the domain 'douglascountuil.gov', a typo for the 'douglascountyil.gov' the other six use, and a domain check drops it rather than shipping a dead address or silently correcting a character in someone's contact detail. HOME ADDRESSES NEVER SHIP, and the yearbook prints one for every member. The term line does not ship either: it reads 'November 2024' / 'November 2026' with nothing saying whether that is the seating election or the term's end, and both readings contradict the returns for at least one member.
    "vermilion-county-board-members.json": 9,  # Vermilion County Board members keyed by district (9 districts x 3 members = 27 seats). PEOPLE COME FROM THE COUNTY'S OWN MEMBERS PAGE, reached by supplying the intermediate certificate the county's server omits — chased from the AIA URI the county's own leaf certificate names, pinned by SHA-256, and NEVER by disabling verification. NOT from the county's district layer, whose attribute table carries three Name/Party/Elected/Email sets per district and is a 2018 snapshot on a retired e-mail domain: the Coles rule holds, an attribute table on a boundary layer is a snapshot of whenever that layer was published. FOUR TRAPS IN ONE TABLE, each handled rather than absorbed: ONE NAME IS INVERTED (twenty-six rows read "First Last" and one reads "Auter, Tara"), so the comma form is flipped and the flip is PRINTED on every run rather than becoming a silent edit to somebody's name; ROLES RIDE THE NAME CELL ("Steve Miller (Chairman)"), split to their own field because leaving them in ships a name nobody is called; ONE E-MAIL CELL CARRIES A PHONE TOO, so the cell is tokenised rather than taken whole; and SURNAMES ARE NOT KEYS, because District 4 seats two McFaddens. The board office's own address and phone ride each district's roster block, so a card for a member with no direct line still says where to reach the board. The county's own "Elected/Appointed" year ships as "on the board since", labelled that way because the county does not say which of the two it was. The board LANDING page's staff table is never read for members: it lists the HR director and the office manager alongside the chairman.
    "wayne-county-board-members.json": 7,  # Wayne County Board members keyed by district (7 districts x 2 members = 14 seats). Scraped weekly from the county's own board page — the SAME page the district composition was read from, so re-scraping it every week is this county's redistricting tripwire (the builder fails if a district's re-read precinct list stops matching the shipped composition; District 7 is checked as the known subset, since the page has never named Fairfield 1 or Fairfield 2). Each district's paragraph names two members with an optional Chairman or Vice Chairman title — Daryl Hargrave (D1) and Gene Kollak (D7) hold the two titles the page states. No e-mail, phone, party or term is published, so none ships.
    "clay-county-board-members.json": 14,  # Clay County Board members keyed by district LETTER (A-N, one member each = 14 seats; 14/14 with a phone, as the county prints it, dots and all). Scraped weekly from the county's own MEMBERS page — an Elementor team widget per member, the county's maintained list (the Alexander rule), each position line stating the district ('County Board - District K'; the Chair's card carries the office in the same line: 'County Board Chair - District C'). Each card prints its phone TWICE (an image-overlay copy and a content copy) and the scraper requires the two to agree. ROLES COME FROM TWO COUNTY SURFACES THAT MUST AGREE: the Chair is marked on his own card AND named in the county-board page's Chairman tab, and the build fails if they name different people. The VICE CHAIRMAN is named only in that prose, under a different form of her name — the board page writes 'Barbara McGrew', the members page prints 'Barb Mcgrew' — so the role joins by surname, required unique, with the join PRINTED on every run; the shipped name is the members page's exactly as printed, lowercase g included (the Fulton 'Karl WIlliams' rule). THE SAME WEEKLY RUN IS THE REDISTRICTING TRIPWIRE: it re-reads the county-board page's Districts section letter by letter and fails if it stops matching the composition compiled into build_clay_boundaries.py. What it cannot trip is stated in the builder: the village-limits line splitting Clay City between districts A and B came from the Clerk's e-mail, and no page re-read can check a line the page never draws. No e-mail, party or term ships because the county publishes none. District C's letter was the one omission on the county's pages for weeks — Clerk Britton named Joe Goodman for it by e-mail (2026-08-24), and the county's own members page now states it too, which is the version that ships.
    "st-clair-precinct-polling.json": 3,  # St. Clair's precinct -> polling place table — 149 precincts with a place and address, plus the county's one bare East St Louis feature carrying the Clerk's explanation that the city runs its own Board of Election Commissioners. THE SOURCE IS A CLERK-SENT DOCUMENT, NOT A PAGE: the county's published list groups precincts under combined human labels ('Belleville9,10, 12 & 16') that cannot be matched to single precincts without interpreting prose, which is what the st-clair-precinct-polling-places gap recorded since 31 Jul 2026. Asked for the same assignment one row per precinct, Sarah Hermsdorfer of the Clerk's Election Department sent 'St Clair County IL Polling Places.xlsx' on 2026-08-26; it is archived as a faithful CSV transcription under data/source/raw/ and joined by scripts/build_stclair_precinct_polling.py against the county's own precinct layer (prec_name2), so the shipped keys are exactly the strings the app looks up. THE SHEET'S HEADER IS A TRAP — its first column is titled POLLING PLACE and holds the PRECINCT name. One enumerated alias (the layer's 'Ofallon n' for the Clerk's 'O FALLON n'); the build fails on any other mismatch, any double-join, or any unmatched layer precinct besides East St Louis. FOUR ROWS CARRY THE CLERK'S ADVANCE NOTICE: Centreville 2, 5, 7 and 8 are set to move to the new Cahokia High School (805 Camp Jackson Rd) once the County Board votes on the change — the current assignment ships, the note rides those cards, and the file is stale for those four the day that vote passes. Operator step like Logan's and Carroll's: re-run when the Clerk sends an update.
    "boone-district-officials.json": 4,  # The office and board of Boone County's five park and library taxing bodies, keyed by the same district name the two parcel-dissolve boundary files carry — the card joins the two, and scripts/build_boone_district_officials.py REFUSES TO WRITE unless the roster's keys are exactly the districts drawn, because a name drifting apart would empty a contact block silently while the polygon still drew. THE CLERK'S YEARBOOK OWNS THE OFFICE AND OWNS NO BOARD AT ALL, which is the correction this build made to its own gap record twice over. The booklet supplies the address, telephone, fax, website and agency head for all five; every BOARD comes from that body's own published page or document, because the booklet carries none for the three seated in Winnebago and is SUPERSEDED for the two seated in Boone — its Ida board predates that library's own 28 July 2026 officer election, and its Belvidere Park board predates a fifth commissioner the district itself lists. A VACANCY SHIPS ONLY WHERE THE BODY STATES ONE IN WORDS (Ida's list prints 'Trustee: Open'), never from seats-minus-rows: an earlier build derived Belvidere Park's that way and published '1 of 5 seats is vacant' about a district whose own page names five, because a markup quirk had hidden one. A roster short of the seats a body states now breaks the builder's floor instead. Rockford Park District is read from its plain /board page, NOT the ?format=json endpoint its robots.txt disallows. North Suburban's trustees come from a linked PDF, so the card cites the DOCUMENT and its Last-Modified date and the scraper warns if it predates the last April consolidated election. One address does not ship: Belvidere Park's Vice-President's row links one mailbox and displays another, and nothing settles which is current. Four Belvidere Park staff parse cleanly and are dropped on purpose — employees, not the governing body — and Ida's director ships as the library's own 'Library Director' with the library's own mindyl@ rather than the yearbook's 'Executive Director' and mistyped mindlyl@.
}
