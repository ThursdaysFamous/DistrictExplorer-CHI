...
GEOMETRY_FILES = {
    ...
    "carroll-library-districts.json": (7, 7),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py (clipped to the county, simplified 5 m). NOT a county or library publication: copyrightText and the item description are both empty and every other attribute is a broadband service metric, so every card built on it names the publisher. Carroll's 7, and the county that validates the whole source: these are exactly the seven library tax lines the County Clerk's own Tax Code by District Listing names, and the builder refuses to write if they ever stop matching. Shannon and Lake Carroll correctly sit in NO library district, which the Clerk's tax codes independently confirm.
    ...
    "lee-library-districts.json": (11, 11),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py (clipped to the county, simplified 5 m). NOT a county or library publication: copyrightText and the item description are both empty and every other attribute is a broadband service metric, so every card built on it names the publisher. Lee's 11. Five are seated in neighbouring counties and reach across the line; only the Lee slice is drawn, so a reader on either side is answered by their own county's entry.
    ...
    "st-clair-library-districts.json": (19, 19),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py (clipped to the county, simplified 5 m). NOT a county or library publication: copyrightText and the item description are both empty and every other attribute is a broadband service metric, so every card built on it names the publisher. St. Clair's 19, the densest of the six. East Saint Louis, Belleville and O'Fallon are municipal libraries; New Athens, Marissa and Freeburg are districts.
    ...
    "stephenson-library-districts.json": (5, 5),  # Boundaries from the Illinois Broadband Office / Connected Nation IL_Boundary_Layers service, layer 11, pre-built by scripts/build_statewide_library_districts.py (clipped to the county, simplified 5 m). NOT a county or library publication: copyrightText and the item description are both empty and every other attribute is a broadband service metric, so every card built on it names the publisher. Stephenson's 5. Pearl City Public Library District appears here AND in Carroll's file, each with its own county's slice — the district straddles the line and neither county's reader loses their answer.
