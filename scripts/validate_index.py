...
GEOMETRY_FILES = {
    ...
    "mercer-county-outline.json": (1, 1),  # Mercer County outline — gap-location geometry only, not a dispatched county (see bureau-county-outline.json for the pattern). Ships so mercer-county-board-districts attaches to its ground.
...
ROSTER_FILES = {
    ...
    "tazewell-precinct-polling.json": 4,  # The SIX precincts Tazewell County's own polling layer cannot answer. Its precinct layer points Cincinnati 02/05, Elm Grove 02, Morton 01 and Pekin 01/06 at facility ids 43, 50 and 54, none of which appear among the 49 voting locations it publishes. Deputy Clerk & Election Supervisor Vanessa L. Reynolds sent the Clerk's own countywide polling listing (19 townships, 82 precincts, 49 places) by e-mail on 2026-08-17 in reply to the question, and it names all six buildings with addresses. KEYED BY PRECINCT NAME, not by facility id like Whiteside's — the county's data forces it: id 50 is shared by Cincinnati 02/05 (Pekin Bible Church) and Elm Grove 02 (Grace Baptist Church), and id 54 by Pekin 01 (First Christian Church) and Pekin 06 (Miller Center), so precincts voting at DIFFERENT buildings share one id and no facility-keyed file could express the answer. Consulted ONLY where the county's own layer has no match, so publishing those buildings upstream retires it silently. Deliberately narrow: five precincts where the county's live layer and the Clerk's listing name DIFFERENT buildings are left showing the county's answer and asked of the Clerk instead — its layer was edited 2026-08-14 and is not stale, so overriding published county data from an e-mailed document would be guessing which county source is right.
