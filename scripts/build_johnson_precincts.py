...
county's ELECTION AUTHORITY publishes somewhere else — results.gbsvote.com, a
results platform carrying fourteen Illinois counties that this project had not
catalogued until 2026-08-21 — and a county's certified canvass answers both
questions a join needs without the county's own site being reachable at all.
...
OUT_PRECINCTS = os.path.join(REPO_ROOT, "data", "app", "johnson-precincts.json")
...
def fail(msg):
    print("johnson-precincts: FAIL — %s" % msg, file=sys.stderr)
    sys.exit(1)
