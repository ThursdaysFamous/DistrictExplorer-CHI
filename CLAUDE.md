...
**Coverage is no longer "the Chicago collar."** It began as the seven metro counties and has grown one contiguous county at a time to **35 dispatched counties** — north to the Wisconsin line, west to the Mississippi, and south through the Metro East to Randolph on the Missouri border. The authoritative list is `DISPATCH_COUNTY_FIPS` in `scripts/build_metro_outline.py`; `validate_index.py` fails the merge gate if a county gains a dispatch entry without joining it. Adding a county adds **dispatch entries and roster rows, never new layers** (`docs/EXPANSION_GUIDE.md` Part 2) — with one shape that adds neither: a county electing its board AT LARGE has no district geometry, so its members ride the County card via `data/app/il-county-commissioners.json` instead of the `county-board` layer.
...
- Scheduled workflows: `update-ilga-roster.yml` (Mon 13:00 UTC); `update-congress-roster.yml` (Mon 13:00 UTC); `update-cpd-roster.yml` (Tue 13:00 UTC); `update-ccpsa-roster.yml` (Wed 13:00 UTC); `update-will-county-board-roster.yml` (Thu 13:00 UTC); `update-kendall-county-board-roster.yml` (Thu 14:00 UTC); `update-mchenry-county-board-roster.yml` (Thu 15:00 UTC); `update-kane-county-board-roster.yml` (Thu 16:00 UTC); `update-lake-county-board-roles.yml` (Thu 17:00 UTC); `update-ccbr-roster.yml` (Fri 13:00 UTC); `update-county-clerk-roster.yml` (Sat 13:00 UTC); `update-dupage-county-board-roster.yml` (Sun 13:00 UTC); `update-winnebago-county-board-roster.yml` (Wed 15:00 UTC); `update-sangamon-county-board-roster.yml` (Tue 15:00 UTC); `update-livingston-county-board-roster.yml` (Tue 14:00 UTC); `update-municipal-officials.yml` (Wed 14:00 UTC); `update-dekalb-county-board-roster.yml` (Fri 14:00 UTC); `update-ogle-county-board-roster.yml` (Sat 14:00 UTC); `update-stephenson-county-board-roster.yml` (Sun 14:00 UTC); `update-carroll-county-board-roster.yml` (Mon 14:00 UTC); `update-lee-county-board-roster.yml` (Tue 16:00 UTC); `update-rock-island-county-board-roster.yml` (Wed 16:00 UTC); `validate-sources.yml` (1st of month 14:00 UTC); `update-lasalle-county-board-roster.yml` (Sun 15:00 UTC); `update-woodford-county-board-roster.yml` (Mon 16:00 UTC); `update-logan-county-board-roster.yml` (Mon 17:00 UTC); `update-boone-county-board-roster.yml` (Tue 17:00 UTC); `update-grundy-county-board-roster.yml` (Wed 17:00 UTC); `update-henry-county-board-roster.yml` (Thu 18:00 UTC); `update-peoria-county-board-roster.yml` (Fri 15:00 UTC); `update-tazewell-county-board-roster.yml` (Fri 16:00 UTC); `update-iroquois-county-board-roster.yml` (Sat 15:00 UTC); `update-county-commissioners-roster.yml` (Sat 16:00 UTC); `update-dewitt-county-board-roster.yml` (Sun 15:00 UTC); `update-washington-county-board-roster.yml` (Sun 16:00 UTC).
...
- `fleet-status.yml` — weekly (Mon 15:00 UTC) fleet aggregator, **Chicago repo only**: `scripts/fleet_status.py` reads `metros.json`, checks every fork's deploy/engine-pin/roster state, diffs each fork's layer roster against `docs/DATA_LAYER_GUIDEBOOK.md`'s coverage map, and diffs each fork's shipped `data/app/coverage-gaps.json` against the guidebook's gaps block. That last one is the **only** gate on sibling gap drift: the guidebook lives here, so a sibling's file is generated here and lands there via a bump PR — it has no local `--check`. Reports on a single standing issue.
