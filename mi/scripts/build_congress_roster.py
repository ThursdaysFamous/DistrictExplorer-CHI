...
data/state/state.json (Michigan: USPS MI, 13 seats), which overrides the
...
A weekly GitHub Action (.github/workflows/update-mi-congress-roster.yml) reruns
...
# A state instance parameterizes this script by carrying data/state/state.json
# ({"usps": "MI", "house_districts": 13}) instead of editing the constants
# above: the reference fork carries no data/state/, so the IL defaults hold
# and this block is inert there — one script, byte-identical, in every fork.
...
if os.path.exists(_STATE_CONFIG):
    ...
    STATE = _state_cfg.get("usps", STATE)
    EXPECTED_DISTRICTS = int(_state_cfg.get("house_districts", EXPECTED_DISTRICTS))
